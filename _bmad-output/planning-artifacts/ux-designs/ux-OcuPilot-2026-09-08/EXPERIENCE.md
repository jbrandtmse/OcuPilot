...
| "messages.log" · "Clear filter" | messages.log viewer (Story 6.14): its side-bar entry and screen title, the second Logs entry (`:85`, `:163`); and the control that clears the severity-chip filter (`:411`), published here rather than reused because `:403` names "Clear filter" only as a `button-text` example in Component Patterns and no Fixed strings row has carried it (DW-1109) |
