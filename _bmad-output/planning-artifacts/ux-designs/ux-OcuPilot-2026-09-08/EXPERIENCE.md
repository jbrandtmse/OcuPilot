...
| "Task history" · "Started" · "Completed" · "Status" · "Result" · "Task ID" · "Error date" · "Logged" · "Contains" · "User-defined tasks only" · "No task runs match." | Task history (all) side-bar entry and screen title, the fourth Tasks entry; its column headers and its detail dialog's field labels beyond the shared Name, the namespace switch's "Namespace", the Processes row's "User" and "Routine" (`:319`) and the Application errors row's "Error number" (`:325`); its search field's label and its user-defined-only checkbox's label above the table; and its empty state, shown once Search finds nothing |
