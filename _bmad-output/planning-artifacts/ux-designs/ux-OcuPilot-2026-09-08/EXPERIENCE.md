...
| "alerts.log" · "<n> of <N>" · "Recent entries are unavailable — showing the file tail." | alerts.log viewer (Story 6.13): its side-bar entry and screen title, the first Logs entry (`:84`, `:163`); the polite match count the sticky search announces (`:408`, `:647`); and the one line that distinguishes this screen from messages.log — the monitoring API's half of the merge failed while the bounded tail answered, so the screen keeps its rows rather than showing a refusal (`:408`) |
