...
| Audit database viewer | Logs | side-bar › Audit database · cross-link (dashed) from Auditing configuration | criteria form: time range, source, type, name, user, pid, namespace, authentication, text; agent-marker filter (FR-61, LG-02 = SS-14) | list (server criteria) | 3 · P0 |
...
| Processes list | OS management | side-bar › Processes | filter, max rows, persisted sort, auto-refresh (FR-54) | list | 1/3 · P0 |
...
| Task schedule | Tasks | side-bar › Task schedule | Task Manager status and control (a warning banner with Resume while it is suspended); last and next run; filter; auto-refresh (FR-48) | list | 1/3 · P0 (control 4) |
...
| SSL/TLS configurations list | Security and secrets | side-bar › SSL/TLS | list (FR-42) | list | 3 · P0 |
...
| OAuth 2.0 | Security and secrets | side-bar › OAuth 2.0 | one screen, five tabs: Client server descriptions · Client configurations · Resource servers · Authorization server (view) · Server client descriptions `[ASSUMPTION: five lists as tabs of one screen rather than five side-bar entries]` (FR-44) | list | 3 · P0 |
...
| "You need %Admin_Secure to open Security and secrets." | "Access denied" |
...
| empty-state | lists, viewers | One `{typography.title}` sentence naming the scope ("No locks in HSCUSTOM."), a second line saying what to do next, and, where one exists, the single primary action ("Create web application"). On write-capable lists the second line invites the agent: "Or ask the agent: create an SSL/TLS configuration for outbound HTTPS." Keyboard hints render as kbd chips (Ctrl/Cmd+K to find a screen; Alt/Option+Down for a row's menu). A 32 px icon per `DESIGN.md`, no illustration. A refused document or a permission-denied screen is not an empty-state — it shows the refusal. Version-mismatch and no-administrative-privileges notices use this shape with a banner (error) and their link. |
...
| Expired password | Form login | the server's expiry error | "The password for <user> has expired. Change it in the classic portal, or run the command in the README to clear the expiry." with both links (UJ-5). `[NOTE FOR ARCHITECTURE]` a fresh Community container expires `_SYSTEM`'s password on first login, so a user following the README meets this state first; consider unexpiring it at install (the project's own CLAUDE.md carries the one-liner) so the first screen never sends the user to the classic portal | password changed → sign in |
| Version mismatch | shell | `/info` absent or `apiVersion` ≠ 2 | blocking notice (empty-state shape with a banner (error)) naming the mismatch ("This instance's admin API is version 1; OcuPilot needs version 2.") and a link to the classic portal; the rail, side-bar and command-box are inert; no area screen loads | none in-app |
| No administrative privileges | shell | privilege map holds no `%Admin_*` | "no administrative privileges on this instance" (empty-state shape with a banner (error)) and a sign-out link; never presented as a version mismatch | Sign out |
...
| Request refused | any call | 403 | inline message (`role="alert"`) naming the missing resource with the action ("You need %Admin_Manage to terminate processes."); the screen's data stays | — |
...
| Task Manager suspended | Task schedule | the Task Manager is suspended | warning banner above the table "The Task Manager is suspended — no scheduled task will run until it is resumed." with Resume (privilege-gated per Privilege Gating); rows still list | Resume |
...
- **Status messages (WCAG 4.1.3).** The transcript is `role="log"` (polite, `aria-label="Conversation"`, `tabindex="0"`), so the agent's reply, each tool-call card and each proposal are announced by construction and a card's status change is a text update. Polite `role="status"`: the Test connection result, "Saved", the search "n of N", the filter count, the command-box count, the toast region, the change highlight ("Updated: <entity> <action>"), the lock banner, and the connection-state segment on transitions only. `role="alert"` (or focus moved to it): the sign-in failure, the instance-unreachable banner, the generic error, a 403, the form error summary. The countdown is announced once, at 1:00 ("One minute left to confirm"), never per second, and is otherwise not live; the auto-refresh stamp and refresh ticks are never announced. Kill-switch and enforced read-only banners are assertive on appearance. Agent navigation: the announcement is committed to the log, the route changes after it has been dispatched (about 1 s), and the new heading's announcement reads "<title> — opened by the agent; Back returns".
