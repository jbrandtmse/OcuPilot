...
| meter | System usage, Database details | Label · value · unit, with the dashboard's own state (normal / warning / alert) shown as a word and colored `{colors.success}` / `{colors.warning}` / `{colors.error}`. Refreshes on the screen's interval; shows "—" with a skeleton until a value arrives; never animates the needle. |
