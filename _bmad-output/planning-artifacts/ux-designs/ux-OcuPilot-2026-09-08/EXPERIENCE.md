...
| "Task schedule" · "Last run" · "Next run" · "No scheduled tasks on this instance." | Task schedule list side-bar entry and screen title (`:165`, `:102`); its column headers beyond the shared Name and Type, with Namespace reusing the namespace switch's accessible name (`:333`); and its empty state, which names the instance because the read is instance-scoped and its rows span namespaces, and whose second line is the Web applications row's (`:238`) |
