...
- **Rule:** Epic 5's first three writes were all one shape — a `PUT` of a complete body merged over a fresh `GET` — and the write path hardcoded it. Some vendor operations have no such shape: `Task.CRUD` exposes resume only as the bodyless `RESUME` request type, and AD-3 already names `Task.Manager`, `Process` and `Lock` as "action-style with trivial or empty bodies". An **action-style write** is therefore a first-class kind, not an exception:
  ...
  **The subject must be adequate, and adequacy is structural rather than a review convention.** It is not "the fields the slice cares about"; it is **the fields whose movement invalidates the reviewed intent**, and two classes are definitionally in that set:

  - the **scoped target identity** (AD-13), and
  - **every field the action's own precondition reads** — `State` for suspend and resume, because a subject of `{Pid}` alone would let a confirm succeed against a process somebody else had already suspended, which is "state moved under you" in its purest form.

  An empty subject is refused, and that is necessary but nowhere near sufficient; the build fails when an action tool declares a subject omitting its identity or its precondition field. **The subject is validated by the same builder that validates `fingerprintExcludes`** — a name neither the write tool nor the read declares is refused identically, through one extraction consumed by both. A second validator beside the first is the DW-1206 defect wearing a new hat: two sibling declarations checked against different sets, fixed three stories earlier by extracting the union once.
