...
| Whether AD-10's privilege-grant set covers WRITE on `%DB_IRISSYS` and `%DB_IRISSECURITY` in a role's resource grants (DW-1512, **decision-pending**) | The shipped predicate escalates only `%All` and `%Admin_*` resources, at any permission; `%DB_IRISSYS:WRITE` is a known self-escalation primitive (Conventions > IRIS security objects). Widening changes Story 8.2's role-picker verdicts and the implicit `%DB_*` roles, so it is decided once rather than per story; Story 8.3 ships the existing predicate | The Epic 8 merge-gate decision sheet |
