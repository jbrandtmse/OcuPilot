...
| Secrets | Write-only through UI and API; never returned, never logged, never in a proposal's stored arguments. Redaction is **schema-driven** — a field is secret because its descriptor says so (FR-21), not because its name matched a pattern; a wallet secret field named `Value` defeats any matcher. A name-pattern matcher runs as a second, backstop layer that can only add redaction, never remove it. The pattern is the case-insensitive suffix `(password|passwd|pwd|secret|apikey|privatekey|token)$`; at build time a derived field with a string placeholder matching it that is not classified secret fails the build (AD-3). |
