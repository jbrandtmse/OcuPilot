...
- **Rule:** Get-merge-put is **not** a property of the admin API and must never be assumed. Of 47 `RunPut` implementations only 19 call `MergeJsonAndProperties`; the 28 that do not include most Release 1 editors — `Security.User` (FR-35), `Task.CRUD` (FR-53), `Security.X509Credential` (FR-43), `Security.Audit.Event` (FR-47) and all four `Security.OAuth2.*` (FR-44). Sending only the changed fields to one of those erases every field omitted. `Security.Resource` (FR-40) and `Wallet.Secret` (FR-46) also call no merge helper, but each keeps a field its body omits (measured 2026-09-23: `Security.Resources.Modify` receives only the keys the body carries; a `Wallet.Secret` body without `Secret` kept the stored value); OcuPilot sends them the complete set all the same, because the rule is uniform.
...
- **Rule:** The Release 1 prohibited set is refused by the write path on the instance and is never advertised as a tool. Governance can disable a permitted tool; it can never enable a prohibited one. A later stage adding prohibited actions adds them here, not to a policy file. The set is defined by **effect, not by verb** — the first draft enumerated deletes and disables, and two whole classes of privilege change slipped between them:
  ...
  - **Weakening or repointing one of OcuPilot's own web applications**: making it reachable without signing in (`PROHIBITED.UNAUTHENTICATED`), clearing its authorization resource (`PROHIBITED.AUTHORIZATION`), or changing the code that answers its address - `DispatchClass`, `NameSpace`, the Python application and callable, `Package`, `SuperClass` (`PROHIBITED.DISPATCH`). The same serving-path self-protection family as the own-application roles above [AMENDED 2026-09-24, Story 9.2 spec gate, orchestrator ruling applying the owner's 2026-09-23 "developer tool first" direction, Rule 20, DW-1207: these three arms applied to every web application; on any other application each change is now **permitted at the strongest confirmation** - an agent proposal is minted destructive and names the effect ("reachable without signing in", "no authorization resource", "a different class now answers this address"), a person's Save shows a consequence line. `Path` and the Python location stay caller-unnameable under AD-21 whatever the application].
...
- **Rule:** Epic 5's first three writes were all one shape — a `PUT` of a complete body merged over a fresh `GET` — and the write path hardcoded it. Some vendor operations have no such shape: `Task.CRUD` exposes resume only as the bodyless `RESUME` request type, and AD-3 already names `Task.Manager`, `Process` and `Lock` as "action-style with trivial or empty bodies". An **action-style write** is therefore a first-class kind, not an exception:
  ...
  - **The request type is declared per tool, not assumed.** The tool names the admin request type it issues; `PUT` is the default a merge write keeps, never a constant the kernel enforces. The same is true of the type the mint reads for its fresh read. That read may be a list type filtered by a declared read parameter, where the one-object type does not answer a field the diff shows: the target is the one row whose declared key equals the id, and no such row reads as absent [AMENDED 2026-09-23, Story 9.1 spec gate, Rule 20, DW-1576: the audit-event reset's `GET` answers only `{Description, Enabled}`, while `LIST names=<EventName>` answers `Total`, measured on this build].
