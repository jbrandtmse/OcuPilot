...
- **Rule:** Get-merge-put is **not** a property of the admin API and must never be assumed. Of 47 `RunPut` implementations only 19 call `MergeJsonAndProperties`; the 28 that do not include most Release 1 editors — `Security.User` (FR-35), `Task.CRUD` (FR-53), `Security.Audit.Event` (FR-47) and all four `Security.OAuth2.*` (FR-44). Sending only the changed fields to one of those erases every field omitted, unless a measurement below says otherwise; each `Security.OAuth2.*` endpoint is measured by the Epic 12 story that builds its editor. `Security.Resource` (FR-40), `Wallet.Secret` (FR-46) and `Security.X509Credential` (FR-43) also call no merge helper, but each keeps a field its body omits (measured 2026-09-23: `Security.Resources.Modify` receives only the keys the body carries; a `Wallet.Secret` body without `Secret` kept the stored value; measured 2026-09-24 on a throwaway: an `X509Credential` PUT carrying only `OwnerList` kept `PeerNames` and `CAFile`, because its `RunPut` sets each only when the body defines it); OcuPilot sends them the complete set all the same, because the rule is uniform. [AMENDED 2026-09-24, Story 12.1 spec gate, Rule 20: `Security.X509Credential` moved out of the erasing list on measurement]
...
- **Rule:** The instance mints a proposal carrying the tool, the resolved arguments, a **scoped target identity** (AD-13), a target fingerprint, the computed diff, and a single-use token. Confirm re-reads the target, compares the fingerprint, refuses on mismatch, executes from the **stored** arguments, and burns the token. Proposals expire after a server-side constant of 10 minutes. A write tool call arriving without a valid, unexpired, unburned proposal token is refused by the write path itself, not by the caller. Three things the first draft left open, each of which two slices would otherwise answer differently:
  ...
  - **The fingerprint covers the complete property set the write will send** (AD-4), excluding fields the endpoint itself mutates as a side effect — a task's next-scheduled time, a last-modified stamp. For a **merge write** a slice does not choose its own fingerprint fields; the descriptor declares the exclusions and the default is "everything else". An **action-style write** (AD-51) sends no payload, so there is no property set to exclude from and the selection is inverted there: the tool declares the subject the fingerprint covers, under AD-51's adequacy rule. That is the only kind for which it is inverted, and AD-51 is where the rule lives.
...
- **Rule:** There is no service account, and the only elevation anywhere on the request path is the one named in AD-9. [AMENDED 2026-09-19, Story 5.4 spec gate, DW-444: the clause read "no elevation anywhere on the request path". Refusing a disabled account at authentication needs that account's `Enabled` flag, which an unescalated caller cannot read, and the owner decided at Epic 4's merge gate to refuse it rather than ship the hole. The escalation is one method, behind its own privileged routine application, reading one flag for the authenticating user and nothing else.] Every gate is evaluated in the calling process at the moment of the call, against the privileges the screen descriptor names — **a set of `(resource, permission)` pairs, never a single resource.** One name is not enough on 2026.2: the security APIs need `%DB_IRISSYS:R` *and* `%Admin_Secure:U` together, so a descriptor field that holds one string cannot express what the screen actually requires. The descriptor declares the full set, the gate requires all of it, and a denial names the pair that failed so the UX can say which privilege is missing. [AMENDED 2026-09-21, Story 5.8 code review, DW-1208 (owner-decided at the merge gate): **a write tool's pair set is the screen's own declared set, and an administrative resource on it is required at `USE`, never `WRITE`.** IRIS's built-in `%Admin_*`, `%Service_*` and `%Development` resources are system resources that carry no `WRITE` permission -- verified on the instance, where every shipped role grants them `:U` and only `%DB_*` resources carry `:R`/`:RW` -- so a `WRITE` pair is a gate only a `%All` holder can pass, which would invert the least-privilege story. `USE` is also exactly what the vendor's own editor checks (`%CSP.Portal.Application.CheckSecurity` is `Quit $System.Security.Check(pResource,"USE")` against a suffix-less `Parameter RESOURCE`), which is what makes "the same 403 the editor would give" satisfiable by construction. An OcuPilot-owned resource carrying its own `WRITE` was considered and rejected: it is a second authorization model that must be kept in step with the vendor's, and on divergence OcuPilot either refuses a write IRIS would allow or advertises one it will refuse.] The full tool set is always advertised; a 403 from a tool is identical to a 403 from a screen and is reported, never retried. The one permitted elevation is AD-9's, and it is not in effect while any tool, port or provider code runs.

  **A write tool may declare pairs beyond its screen's set only when the vendor class it reaches writes a database the screen's read does not; it declares them itself, and a caller without them is refused by name before any port call** [AMENDED 2026-09-23, orchestrator ruling at Story 8.8's implement halt]. The one case today: the device tools' `%DB_IRISSYS:WRITE` (`Config.Devices` writes the system database; a principal holding only the Devices screen's `%Admin_Manage:USE` and `%DB_IRISSYS:READ` was refused by the vendor on every write, measured 2026-09-23). The screen's own set stays read-only, so the Devices list and the rest of OS management remain open to users who only read; the next exception names itself here. **A write tool may also declare a pair its screen's set lacks when the vendor endpoint's own `ResourcesOR()` names that resource for the request type the tool issues, measured on the instance; it declares it itself and the mint refuses a caller without it by name before any port call, exactly as above** [AMENDED 2026-09-24, orchestrator ruling at Story 12.2's spec gate, Rule 20]. The one case today: `permissions.users.revoketokens`' `%Admin_OAuth2_Registration:USE` (`Security.OAuth2.Server` `REVOKE`; a caller without it was answered 403 on `ocupilot-b-ci`, 2026-09-24), on the Users list whose own set is `%Admin_Secure:USE` and `%DB_IRISSYS:READ`.
...
- **Rule:** OcuPilot's **globals** — and only its globals — live in a dedicated database guarded by a dedicated resource that no ordinary role holds. **OcuPilot's code does not.** In IRIS, database READ *is* routine-execution permission, so putting the packages behind a resource no ordinary role holds would make OcuPilot unrunnable by exactly the users it is for. Code lives in the install namespace's normal database; data lives behind the resource. The FR-29 acceptance test — a `%DB_<install-namespace>:RW` plus `%Admin_Operate` holder can read and write neither table nor global — is satisfied by protecting the data alone.
  ...
  The storage classes, and **one identity read** (AD-8, DW-444: whether the authenticating user's account is enabled), obtain a role through a **privileged routine application** — a separate application per purpose, each granting only what that purpose needs, and the identity one granting no access to OcuPilot's own data (`$SYSTEM.Security.AddRoles(<app>)`, `Security.Datatype.ApplicationType` "Routine"), always inside a `New $ROLES` frame so the role is gone when the frame unwinds. Verified on the instance: `New $ROLES` + `Set $ROLES` restores the prior role set on unwind.
...
- **Rule:** The Release 1 prohibited set is refused by the write path on the instance and is never advertised as a tool. Governance can disable a permitted tool; it can never enable a prohibited one. A later stage adding prohibited actions adds them here, not to a policy file. The set is defined by **effect, not by verb** — the first draft enumerated deletes and disables, and two whole classes of privilege change slipped between them:
  ...
  - **Granting application roles on OcuPilot's own web applications.** Setting application roles (`MatchRoles`, `Roles`) on the API, the static application or any other web application OcuPilot's installer creates: `%All` there would make every later request - including every turn job - run elevated, quietly falsifying AD-8. It is self-protection, the same family as disabling the serving path, and keeps the `PROHIBITED.PRIVILEGEGRANT` code. [AMENDED 2026-09-23, owner decisions "I want to be able to grant those other roles, so grant %All" and "Web applications should allow %All as well, remember this is a developer tool first": was "Granting privilege through any path" - application roles on any web application, adding a role to a resource, or adding `%All` or any `%Admin_*` role to any user or role - with privilege grants not proposable at any confirmation level.] **Every other privilege grant is permitted, at the strongest confirmation**: granting `%All`, an `%Admin_*` role or any role carrying one to a user or a role, an escalating resource grant to a role, and a privileged application role on any other web application go through the one tool (AD-55) with no caller-scoped predicate. An agent proposal that makes such a grant is confirmed as a delete is - the destructive treatment and the typed-name field of Story 14.7 - and its server-computed diff names the privilege; a person's Save shows a consequence line at the field, and where the application is also unauthenticated the one line says that anyone who reaches it runs with that privilege.
  - **A public permission on OcuPilot's own resources** (`%DB_OCUPILOT` and `OcuPilotAdmin`, as the installer names them), refused `PROHIBITED.OCUPILOTRESOURCE` whatever the caller: it would hand every user OcuPilot's protected state or its administration, breaking AD-9. The same self-protection family as the own-application roles above [ADDED 2026-09-23, orchestrator ruling at Story 8.4's spec gate].
...
- **Rule:** A route is `/ocupilot/<area>/<screen>[/<id>]?ns=<NAMESPACE>`. The id occupies one segment and is percent-encoded on write and decoded on read by one shared pair of functions, never by a slice. Encoding and decoding are round-trip tested against a fixed corpus that includes a leading underscore, a slash, a space, a percent sign and a non-ASCII character.
  ...
  **Two spellings of one id are one entity, and which spellings those are is a rule per entity type.** [AMENDED 2026-09-20, Story 5.5 code review, DW-1359: the instance resolves a web-application name case-insensitively and normalizes a trailing slash, so `/API/OcuPilot` and `/api/ocupilot/` both reach the same application. While the reference carried the id verbatim, one target had two keys: AD-34's per-target lock and its sibling cancel did not cover the second spelling, and two concurrent confirms could both win. The canonical form is therefore decided in the identity layer, keyed by entity type and applied where the reference key is built and where a stored key is read back, never by a caller and never by a gate that happens to need it — a safety gate delegates to this rule, it does not own it. Each later write-tool story adds its own type's rule, and a type with no rule canonicalizes to itself. The visible consequence, accepted: the recorded `TargetRef` a proposal, the ledger and the panel show is the canonical spelling, not the one the agent typed.]
...
- **Named case 2026-09-24 (Story 12.2, DW-1616, merge gate).** *A write the vendor does not audit has no vendor event to locate:* IRIS records no audit event for an OAuth 2.0 token revoke (measured, auditing on), so the agent's marker is that write's only record and the correlation is the marker alone. This covers only a write for which the vendor records no event.
...
- **Rule:** Every SQL statement binds caller values as parameters; shape validation never substitutes for binding. **No OcuPilot endpoint accepts a filesystem path from a caller, anywhere** — not only the log endpoints. Where a file is served or read, the caller names it from a **fixed enum** and the directory comes from `$System.Util.ManagerDirectory()` at runtime. The static handler is one of exactly two places that resolve a caller-supplied name to a file or directory, and it does so under both a literal `..` rejection *and* a post-normalization prefix containment check, serving `index.html` for anything unresolved. The second is a web application's Python directory on a WSGI or ASGI create [AMENDED 2026-09-23, orchestrator ruling on DW-1495, Story 8.2 spec gate]: the caller names one directory, never a path - a single segment matching a strict name pattern, a literal `..` refused - which is resolved under `<ManagerDirectory>wsgi/` computed at call time and never cached (`/durable/iris/mgr/wsgi/` on this build, read from the instance), contained by the same post-normalization prefix check with the root itself refused, and sent as both `WSGIAppLocation` and `Path`; the caller's text never reaches the vendor. Both callers pass the one tool (AD-55). Any other location stays a classic-portal action in Release 1.
...
- **Rule:** A screen descriptor declares what its context serializer emits and which fields are secret-typed and therefore never sent. The kernel enforces an instance-wide cap of **200 rows**, settable by an operator, on any context payload — truncating at the cap rather than refusing, and recording the number actually sent so the read tool-call card can show it. Secret-typed fields never leave the instance regardless of the cap.
  ...
  **The kernel adds two members to every screen-context payload, derived on the instance and never taken from the request** [AMENDED 2026-09-24, Story 11.9 spec gate, Rule 20]: `tools`, the wire names of the screen's read tool and of the write tools behind its declared row actions and primary action, resolved through the descriptor and tool registries (a form page declares neither, so its list is empty); and `readOnly`, the `blocked` answer of `Kernel.Restraint.Verdict` for the turn's user and definition, the verdict dispatch enforces (AD-30). A request carrying either is refused `TURN.CONTEXT.INVALID`. Both are added before the total-size bound applies, so they count within it, and a payload with no rows still carries both.
...
- **Rule:** Every `%Api.Admin.*` class is marked `[ Hidden ]` and absent from the class reference; the API itself is **experimental** — named as the contest's intended API at the 2026-09-14 kick-off, specified in `mainspec_v2.json` (`intersystems-community/sysadmin-api-specification`; it declares 185 v2 paths and the 2026.2 instance's own generated spec declares 185, of which **184 match path for path** and the HTTP method set is identical on every one of those 184 - the single difference is `/v2/security/oauth2/revoke` upstream against `/v2/security/oauth2/server/revoke` on the instance. Corrected 2026-09-19 (Story 13.2 plan gate, re-measured by the runner against `ocupilot-b-ci` and the upstream blob `373e8627`): this clause read "its 185 v2 paths match ... path for path", which is off by one), and subject to change until its final form in IRIS 2027.1. That dependency is real and accepted, and it is **contained**:
  ...
  - **The port may complete a call through the vendor's own class only when the admin API cannot carry a call that class can -- it refuses a value the class accepts, takes only a form another AD forbids, has no read, or cannot pass a parameter the class takes -- and only for a named endpoint** [lead widened at the Epic 7 merge, 2026-09-23, so it states what all four cases below share] [AMENDED 2026-09-23, orchestrator ruling at Story 8.4's spec gate]. The one case today: `Security.Resource` `PUT` with `PublicPermission` of no permission. Its `RunPut` normalizes the value with `Security.Datatype.Permission.ExternalToInternal`, whose answer for no permission is `0`, and then refuses `If permsNormalized = 0` with a 400; `null`, `""`, `"N"`, `0` and `"0"` were each refused on a create and on a clear (measured 2026-09-23), while `Security.Resources.Create`/`Modify` accept an empty permission. `AdminPort` completes exactly that call through `Security.Resources` in `%SYS`, under the same `%Admin_Secure` gate and the same verified write, and nothing above the port knows. Any further case is a new named entry here, never a general license. The second case [AMENDED 2026-09-23, orchestrator ruling at Story 8.5's spec gate]: `Security.X509Credential` `POST`, whose template names only server file paths for the certificate and key (AD-21 forbids a caller naming a path); the port completes the import from content through `%SYS.X509Credentials` - verified on this build: a certificate and key set as content saved, the admin API's list read it back with `HasPrivateKey` true and no key bytes, and its `DELETE` removed it. The certificate, key and key password are the tool's secrets: they never touch disk, not even a port-owned temporary file, and never reach the ledger or a log line (AD-35). The third case [AMENDED 2026-09-23, orchestrator ruling at Story 8.6's spec gate]: `Wallet.Secret` `GET` - the admin API has no read for this endpoint (it answers 405, and its list returns only `{Name, Type}`), so `Port/WalletPort` (declared by the wallet write tools, AD-52) composes the read from the vendor's own list plus `%Wallet.Secret.Exists` in `%SYS` and returns `{Type, Usage, RequireTLS, AllowedHosts}` (the name is the query; a `Name` inside `WalletSecretConfig` is refused by the vendor), **never the stored value**. The fingerprint's limit follows and is stated here once: because the value is never read, a change to a secret's value between propose and confirm cannot be detected; only `Type`, `Usage`, `RequireTLS` and `AllowedHosts` are fingerprinted. The fourth case [AMENDED 2026-09-23, recorded under AD-52 at Story 7.8's spec gate and folded here at the Epic 7 merge]: `Process` `TERMINATE` with the error-to-job flag - the admin API cannot express it: `%Api.Admin.Endpoints.Process` `RunTerminate` never passes `SYS.Process.Terminate`'s `SendError`, so the flag FR-55 names is unreachable through `AdminPort` (measured on a throwaway). `Port/ProcessPort` (declared by the terminate tools, AD-52) calls that same documented `%SYS` method the endpoint calls, after repeating the endpoint's guard: its `ResourcesOR()` gate (`%Admin_Operate` at `USE`), its not-found answer (404 for a missing pid) and its refusal of a process that cannot be terminated (409); confirmed with a least-privileged principal.
...
- **Rule:** An agent definition's endpoint is configuration that decides **where the instance's data goes**, so it is treated as such: writing it requires the OcuPilot administrative resource, it is validated to an absolute HTTPS URL (or an explicitly-declared local address for the OpenAI-compatible adapter), it cannot name the instance itself or a loopback or link-local address unless the definition is explicitly marked local, and it is audited as a security change. **A cloud instance-metadata endpoint is refused in every address family, and the marked-local flag does not license one** [AMENDED 2026-09-19, Story 10.3 (DW-1214)] -- marking a definition local says a model sits on your own network, which is never what a metadata service is, so that flag opens no door to one. The guard is a denylist of the addresses providers publish and therefore cannot be complete: a provider that ships a new address is not covered until the list learns it, and the durable posture is an allowlist. Test connection (FR-27) exercises the configured endpoint with a minimal budget and reports what it reached, so a mistake surfaces at setup.
  ...
  **Test connection answers before the Web Gateway does** [AMENDED 2026-09-24, Story 10.5 spec gate]. It is the one provider call made inside a foreground request, and a per-call timeout cannot bound it because `%Net.HttpRequest.Timeout` re-arms on every read (DW-1179). So its one call runs in a short-lived child job (`Kernel.Provider.TestCall`, the only process spawn site beside the turn job of AD-7) that the request waits for against a **wall-clock bound of `g - min(10, g\2)` seconds**, `g` being the gateway's `Server_Response_Timeout` as the installer reads it (AD-17), or 60 when it cannot be read -- so raising that setting lengthens the test. On expiry the request answers its own envelope (`PROVIDER.TESTTIMEOUT`, or `PROVIDER.TESTTIMEOUTLOCAL` for a definition marked local) and records nothing verified; the child writes nothing, so a late answer can never verify a definition. The child is spawned from no escalated frame (AD-9) and the key never becomes a named local on either side (AD-35).
...
- **Rule:** The application error log is reached through **`SYS.ApplicationError`**, the supported API in `%SYS`: the `NamespaceList`, `DateList`, `ErrorList` and `ErrorDetail` queries, and `DeleteByNamespace`, `DeleteByDate` and `DeleteByError`. OcuPilot writes no `^ERRORS` traversal of its own. The underlying store is that global, per namespace, written by `^%ETN`; `Config.Startup.ErrorPurge` is only the **retention setting** — the purge itself is performed by the `%SYS.Task.PurgeErrorsAndLogs` task, so a suspended task means retention silently stops.
  ...
  **The narrower scopes are prefixes of the error's composite id, and all three delete through `DeleteByError`** [AMENDED 2026-09-23, Story 7.10 spec gate, Rule 20]. The scope is the id prefix (namespace; namespace and date; namespace, date and number), each is enumerated at proposal time and deleted as those ids, and `DeleteByDate` and `DeleteByNamespace` are never called. Measured on this build: `DeleteByDate` deletes an error logged after the enumeration, and emptying a date by any method restarts its numbering, so each enumerated entry also carries its `Time` and a reused number no longer matches at confirm. This is the "explicitly refused" half of the by-date clause above, for the vendor method; the by-date scope itself ships.
...
- **Rule:** An action a user performs on **their own account, from the shell, with no agent involved** is not an agent write and does not enter the agent write path. It mints no proposal, takes no confirm token, emits no OcuPilot agent marker, and is never advertised as a tool (AD-40 already makes confirm user-originated; this is the converse case). It calls the **documented `%SYSTEM.*` method for the operation, in the caller's own process**, as AD-8 requires — `$System.Security.ChangePassword($Username, new, old, .tSC)` for the password, which is the call the vendor's own self-service dialog makes.
  ...
  **This is the first direct mutation of an IRIS security object outside a port, and it is a deliberate exception rather than the pattern.** Every other direct `$System.Security.*` call in the tree — `Check`, `CheckUserPermission`, `Audit`, `AddRoles`, `GetGlobalPermission`, across 23 classes outside `Port/` — is a read, an audit emission, or an escalation into OcuPilot's own state, each mandated by AD-8 or AD-9. A **second** such mutation does not inherit this one's permission. It must show all four of the things that earned it here, in its own story's Design Notes: the operation is the user's own, on their own account, with no agent path to it; the caller proves identity to the operation itself (the old password is an argument, which is what separates a self-service change from an administrator's reset); the method is documented and stable rather than `[Hidden]` or experimental, so AD-27's containment is not the thing being dodged; and no port can carry it. A mutation that cannot show all four belongs behind a port, or behind an AD of its own that says why not.


### AD-50 — Per-user preferences are one Kind-discriminated store, read through shell chrome rather than a declared read

- **Binds:** Stories 15.2 and 15.5, and every later per-user preference; AD-5, AD-9, AD-19, AD-36, Conventions › ObjectScript naming and › Concurrent writes
- **Prevents:** a class per preference — favorites, recents, sort, filter, panel width, theme choice — and the opposite error of forcing a per-caller read through a contract that has no per-caller argument
- **Rule:** Everything OcuPilot remembers **about one user** lives in a single `%Persistent` store in the protected database (AD-9), one row per `(UserName, Kind, Name)`, with `Kind` naming what the row is. A new preference adds a `Kind` value — and, where it needs one, a value property — never a new `State` subclass. Two forces make this the shape rather than a preference: the 29-character cap on a `%Persistent` class name (Conventions) leaves 7 characters after `OcuPilot.Kernel.State.`, which `Favorite` and `Bookmark` already exceed; and the drain of writing, guarding, versioning and testing a store per preference is paid once instead of once per polish item. Every write inherits `Kernel/State/Base`'s guarded save, so a preference write is version-conditional like any other state write.
...
- **Rule:** Epic 5's first three writes were all one shape — a `PUT` of a complete body merged over a fresh `GET` — and the write path hardcoded it. Some vendor operations have no such shape: `Task.CRUD` exposes resume only as the bodyless `RESUME` request type, and AD-3 already names `Task.Manager`, `Process` and `Lock` as "action-style with trivial or empty bodies". An **action-style write** is therefore a first-class kind, not an exception:
  ...
  - **It admits no settable fields and sends no body.** Its field list is empty by derivation (AD-3), not hand-typed, and the confirm channel admits nothing (AD-6) because there is nothing to admit.
    A vendor type that requires a fixed body no caller supplies (`Task.CRUD` `RUN`'s `{"RunNow":true}`, measured on this build) takes it from the port, keyed by endpoint and type; the tool still sends no body, and the ledger records none [AMENDED 2026-09-23, Story 7.5 spec gate, Rule 20].
  ...
  **The fingerprint's subject is declared by the tool, not derived by exclusion** [AMENDED 2026-09-22, Story 5.12 spec gate, orchestrator-approved]. AD-6's exclusion model answers the wrong question for an action write, and the failure is not cosmetic: a digest over the whole of a live process's 50-key read refuses the confirm whenever the target has executed a command between mint and confirm, so it **fires hardest for exactly the processes the action exists to act on** — a runaway process is the one whose counters move fastest — and never for an idle one nobody wants to suspend. Measured on this build: across two reads of a working process `CommandsExecuted` moved 1,678 to 6,004,249 and `CPUTime` 0 to 90, while an idle daemon held still over 20 s. AD-6's own purpose survives the inversion intact, because what it prevents is a write against state that **moved under the diff**: for a merge write the diff is the property set, so the fingerprint must cover all of it; for an action write there is no payload and the diff is over state the read can see, so what the user reviewed is *this target, in this state, and this verb*. A counter advancing is not the diff moving — it is the process doing its job.

  **The subject must be adequate, and adequacy is structural rather than a review convention.** It is not "the fields the slice cares about"; it is **the fields whose movement invalidates the reviewed intent** — concretely, **every field the action's own precondition reads, drawn from those the tool's own read type answers**. `State` for suspend and resume, because a subject that omitted it would let a confirm succeed against a process somebody else had already suspended, which is "state moved under you" in its purest form.

  **Identity is not part of the subject** [AMENDED 2026-09-22, Story 5.13 spec gate, orchestrator-approved; this clause previously required the scoped target identity and was wrong]. The fingerprint and the identity answer **two different questions**: the fingerprint asks *"has the state I reviewed moved?"*, while *"which object is this?"* is already answered by the proposal's `TargetRef` (AD-13). Requiring the subject to re-carry identity therefore adds no protection, and where the vendor's read type omits the id it makes a **correct tool unregistrable**: `Task.CRUD`'s `INFO` answers `Type`, `Status`, `Error`, `LastSchedule`, `LastStarted`, `LastFinished`, `NextScheduled` and `Suspended` and **no `Id`**, while `Process`'s `GET` does answer `Pid` — so the old clause bound one shipped tool and not its sibling, purely by an accident of what each vendor read returns. Where the read does answer the id, naming it is permitted and harmless, since it cannot move for a fixed target. The distinction is written here rather than left implicit so the next reader does not re-tighten it.

  An empty subject is refused, and that is necessary but nowhere near sufficient; the build fails when an action tool declares a subject omitting a precondition field its read answers. **The subject is validated by the same builder that validates `fingerprintExcludes`** — a name neither the write tool nor the read declares is refused identically, through one extraction consumed by both. A second validator beside the first is the DW-1206 defect wearing a new hat: two sibling declarations checked against different sets, fixed three stories earlier by extracting the union once.
...
- **Rule:** The write path resolves its target through a port, and **which port is the tool's declaration, not a constant in the kernel.** Until Story 5.13 it was a constant: `Mint`, `Confirm` and `Prohibited` each returned `AdminPort` outright, which was correct while every write targeted an admin API endpoint and silently wrong the moment one did not. AD-48 routes the application error log through `LogSourcePort`, so the write path as it stood **could not express that story's write at all** — and a builder who reads "no `AdminPort` call" as "not a real write" produces exactly the unconfirmed, unaudited deletion AD-48 names.
  ...
  **A tool's declared port may carry out a write the admin API cannot express** through the documented `%SYS` class the vendor endpoint itself calls, repeating that endpoint's guard first [AMENDED 2026-09-23, Story 7.8 spec gate, Rule 20; the case itself moved to AD-27 at the Epic 7 merge]. The one such port today is `ProcessPort`, and its case is AD-27's fourth named case: one list, in the AD that owns the admin-API dependency.
...
- **Rule:** Epic 7's every story is "two callers of one operation", and until Story 7.1 the operation had only one caller. The executed write lived inside `Kernel/Proposal/Confirm`'s transition, and the only instance-write route was `POST /proposal/:id/confirm`, so a screen's own action had nowhere to go but a path of its own. The operation is therefore **lifted out of the confirm path into a callable both callers reach**, and the screen caller gets a user-originated route beside the shipped `GET /screens/:screen/read`.
  ...
  **A write that opens or closes the audit channel also records the observed marking fact the panel's banner reads (AD-15)** [AMENDED 2026-09-23, Story 7.4 spec gate, Rule 20]. The agent's caller takes it from its marker's own answer. The screen's caller emits no marker, so after the write it re-reads, through the tool's port and with the caller's own privileges, the instance auditing flag and OcuPilot's marker event, and records what it observed. The fact is OcuPilot's own state, not a marker, so "emits no OcuPilot marker" below is unchanged; a failed observation records nothing and never fails the write. Measured: until Story 7.4 the fact had two writers, the installer and the agent's confirm, so a screen-side disable left the banner hidden. A tool opts in by declaring it moves the marking (`MOVESMARKING`); Story 7.11's disable of OcuPilot's own events is the second.
  ...
  **Named gap: a screen action on an operation for which IRIS emits no audit event leaves no audit row** [AMENDED 2026-09-24, orchestrator ruling at Story 12.2's spec gate, Rule 20]. The screen's caller relies on the vendor's own event, so where the vendor records none the person's write is unrecorded in the audit database; the agent's caller keeps its AD-15 marker. The one case today: `Security.OAuth2.Server` `REVOKE` (the Users list's revoke-tokens row action), which recorded no event with auditing on (measured on `ocupilot-b-ci`, 2026-09-24). No mechanism is added for it; a later case names itself here.
...
- **Rule:** A screen's Save against an object on the instance and the agent's confirmed write are **two callers of one tool class**. The route resolves the tool for its endpoint, its request type, its settable fields, its payload composition and its port (AD-52), and evaluates AD-10's prohibited set before any port is touched. The screen mints no proposal and takes no confirm token - AD-6's confirmation exists because a *model* proposed the change, and a person pressing Save has already confirmed it - but it inherits AD-10's prohibited set and the write's own validation and fingerprint gates, and AD-8's pair set is the screen's own. **Enforced read-only and the kill switch do not gate a screen Save**: they are the agent's (FR-19, AD-30), FR-20 says every screen keeps working with the agent disabled, and AD-53 states the same for a screen's row actions. [AMENDED 2026-09-23, orchestrator ruling on DW-1494, Rule 20: was "it inherits every gate AD-10 and AD-40 place 'at the write'", which read AD-40's read-only and kill-switch gates onto a person's own Save.]
...
| REST route ordering | `%CSP.REST` matches in file order, so the router preserves three invariants, all enforced structurally over the `UrlMap` by `scripts/check-objectscript.py`: explicit 405 method guards before the catch-all; sub-resource routes before single-segment `:param` routes; and a longer route before a shorter one **whose Url matches its leading segments** -- the constraint binds a prefix family, not the map as a whole, so a route sharing no prefix with any other is unconstrained in position and may be appended at the tail. The first two are also asserted by routing tests; the third is not observable by routing, because a `:param` compiles to `([^/]+)` and never spans a `/`. `OnPreDispatch` authenticates and resolves the namespace once; `Call=` targets are thin wrappers with no business logic. |
...
| Theme | `ocu-theme-dark` on `<html>` is the whole theme. It is set by one framework-free store (`ui/src/app/core/theme.ts`) and selected by one scope (`ui/src/styles/_theme.scss`), which re-points every bare `--ocu-<role>` and `--mat-sys-*` variable to its `-dark` twin. A component draws bare roles and never selects on the theme; a surface whose variant is reversed between modes gets a non-role token pair in `_tokens.scss` rather than a component rule. The choice is per user, on the instance (AD-50), and light is the default before the read settles. Added 2026-09-23 (Story 15.6). |
...
| Theme toggle wiring (DW-39, DW-118, **decided** 2026-09-23, Story 15.6 spec gate) | Both token sets existed in DESIGN.md and `_tokens.scss`, but the dark scope re-pointed only the 30 `--mat-sys-*` variables while components read the bare `--ocu-*` roles, so the toggle was not yet a flag flip. Story 15.6 re-points all 64 roles plus the non-role pairs in the dark scope, and the choice is the `shell` kind's `theme` member on AD-50's store; the Conventions › Theme row governs from here | Story 15.6 |
...
| Whether enforced read-only and the kill switch bar a person's own Save (DW-1494, **decided** 2026-09-23) | Decided no, by orchestrator ruling: the switches are the agent's (FR-19, AD-30), FR-20 keeps every screen working with the agent disabled, and AD-53 says the same for row actions. AD-55's Rule is narrowed to say so | Done |
| A web-application create's caller-supplied `WSGIAppLocation` (DW-1495, **decided** 2026-09-23) | Decided by orchestrator ruling: no AD-21 exception. The caller supplies only a relative directory name, resolved at call time under one fixed root on the instance, with a literal `..` rejected and a strict name pattern enforced, on both callers of the one tool (AD-55). Arbitrary locations stay a classic-portal action in Release 1 | Story 8.2 (routed, floor-blocking) |
| Whether a server-shipped refusal sentence must also be published in EXPERIENCE.md's Fixed strings with a pin (DW-1502, **decision-pending**) | Epic 7's AD-53 (not yet on this branch) publishes a refusal sentence once in Fixed strings and pins the kernel's copy to it; Story 8.2's `PRIVILEGEGRANT` sentence is caller-neutral and reaches the Roles field and the picker from the server only, with no client copy to drift. Either publish and pin it through Epic 7's `RefusalCopy`, or amend AD-53 to exempt a sentence the client never holds | The Epic 8 merge-gate decision sheet, after both epics merge |
| Whether AD-10's privilege-grant set covers WRITE on the system databases in a role's resource grants (DW-1512, **moot** 2026-09-23) | The owner permitted privilege grants at typed confirmation, so no grant is refused for its privilege and the question has no subject | Done |
