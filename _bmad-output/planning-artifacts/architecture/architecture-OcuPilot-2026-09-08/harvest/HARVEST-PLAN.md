...
| `Utils.cls` (1163) — `SwitchNamespace`/`RestoreNamespace`, `ValidateRequired/String/Integer/Boolean`, `SanitizeError`, `ApplyOutputCeiling`, `SurrogateSafeCutLength`, `DecodeUtf8Stream`, `ReadRequestBody` | iris-execute-mcp-v2 | `Kernel/` | Copy. **Correction 2026-09-09:** `Utils.cls` has no `%Atelier` coupling to drop — it extends `%RegisteredObject` with no `Include`, verified by grep; the coupling is in `REST/Base.cls`, `Setup.cls`, `Tests/BaseTest.cls` and six others, none of them copied. Carry it as a forbidden-token guard instead. **Take this early** — everything downstream uses it |
