...
**The 2026-09-14 listing build** is steps 0 and 1 plus a README with install steps and a description. The panel is present in read-only question-answering over those lists if step 2 has begun, otherwise in its configuration-empty state. A screens-only build is admissible for the listing but not for the application.
...
- **SM-2 Listed early, improved daily.** The Open Exchange listing carries a submittable build on 2026-09-14, the published build improves visibly between then and the deadline, and each day of the voting week, 2026-09-28 to 2026-10-04, has at least one user-visible change. Validates FR-64, FR-67, FR-69.
...
- **admin API** — the hidden, undocumented `/api/admin` v2 REST service on the instance. OcuPilot pins version 2.
...
**Consequences (testable):**
- The README, in English, has installation steps that work first time on a clean machine, a description of the product with the UJ-2 and UJ-3 walkthroughs, a video if one is recorded (optional by owner decision), a link to the Ideas Portal idea, and team profile links where applicable.
- The repository is public under the MIT license already in it, which the contest terms' nonexclusive promotional license clause permits.
...
The commitment, the 2026-09-14 listing build and the 2026-09-27 application floor are stated in section 1.1; this section holds the build order they refer to.
