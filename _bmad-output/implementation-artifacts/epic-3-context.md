...
- **Every configuration change is resource-gated server-side and audited** — actor, target, old and
  ...
  silently, so registration is part of the contract; the API web application carries no application
  and no matching roles, asserted by the installer.
...
- **The `form-page` contract** governs both screens: full-page route, one column, sticky Save/Cancel
  bar, validation on blur and on Save, an error summary banner that takes focus on a failed Save and
  links to each invalid field, and an unsaved-changes guard **agent navigation waits on too**.
  Create opens the new editor; edit stays open showing "Saved". A refusal names the resource and the
  action, so each screen needs its own published action phrase, not the envelope's generic reason.
...
- 3.0 closes first; 3.5 and 3.6 build on 3.1–3.4; 3.8's gating and audit cover what 3.1, 3.4 and 3.7
  write. 3.7 is the single enforcement point Epic 4's turn loop and Epic 5's confirm path read, and
  the later per-user read-only toggle sits over that gate rather than adding a second one.
