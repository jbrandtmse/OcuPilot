...
Code review 2026-09-14 (four layers, full tier). 0 decision-needed · 5 patched · 3 deferred · rest rejected.

- [x] [Review][Patch] The two `iris-session.test.mjs` cases billed as covering a refused `SuspendSet` and an unconverged poll asserted `node:assert` against a message literal copied from the caller, so `setTaskManagerSuspended`'s decision was executed by nothing and the two assertions could not fail (Rule 19 — the epic's fourth vacuous assertion). The decision now lives in `taskManagerStateFrom` (`ui/browser/iris-session.mjs`), the caller delegates to it, and the tests drive it. [ui/tools/iris-session.test.mjs]
- [x] [Review][Patch] Nothing pinned the strip's `ocu-list-page-banner` class, the only one that lays the glyph beside the sentence and separates the strip from the table; both existing assertions stayed green without it. The component tier now asserts the whole class list. [ui/src/app/shell/list-page.spec.ts:217]
- [x] [Review][Patch] `ScreenRead`'s banner test stated its Task-Manager-running precondition in a doc comment rather than checking it, so a suspended instance would fail three unrelated assertions with nothing naming the cause. It now asserts `TASKMGRStatus() = 1`; the recorded reason for not doing so ("would need a namespace switch") is wrong — `%SYS.Task` answers from `HSCUSTOM`, verified. [src/OcuPilot/Test/ScreenRead.cls:256]
- [x] [Review][Patch] `filterToSubset`'s catch block dereferenced a row's gridcell unguarded, so a part-rendered row would replace the named failure it exists to produce with a bare `TypeError`; `before.selected` in the AC4 leg could crash where `after.selected` fails cleanly. Both read defensively now, and `before.selected` is asserted non-null so the pair cannot go vacuous together. [ui/browser/list-spec.mjs:87,97]
- [x] [Review][Patch] Two recorded mutation lines were inaccurate (Rule 19, closed in-pass): the `sortBy` mutation reddens `refresh.test.mjs`'s tick test as well as the new store test — `direction` is the slot only the new test carries — and the new `iris-session` pins had no mutation line. Both applied, observed red, reverted byte-identical, and recorded in `## Verification`.
- [x] [Review][Defer] A descriptor's top-level keys are not a closed set, so a misspelled optional key (`banners`, `Banner`) validates, mirrors and ships a screen that never raises its strip — the same silent failure this story closed one level down for `banner.messageKey`. deferred: out-of-footprint, DW-271 (`owner=burndown`); the gap predates the banner for every top-level key and the fix spans both engines. [src/OcuPilot/Screen/Registry.cls:130]
- [x] [Review][Defer] 148 `EXPERIENCE.md` line citations outside `strings.ts` are behind no gate, and this story's Fixed-strings row made six of them resolve to a wrong but plausible row rather than dangling (`command-bar.ts:51`/`:70` cite `:318` for a sentence now at `:336`). deferred: out-of-footprint, DW-272 (`owner=burndown`); the ten in `strings.ts` were repaired as the spec required. [ui/src/app/shell/command-bar.ts:51]
- [x] [Review][Defer] `epics.md:2019` Story 2.8 AC3 still promises "a task suspended after an error appears"; AC2 was amended at origin and AC3 was not, and the shipped screen cannot show suspension. deferred: same root cause as the LIST's `Suspended` coercion — recorded as an occurrence on DW-269 (`owner=4-10`), not re-filed.

**Rejected.** `role="status"` on the strip against "refresh is silent" (**twice raised, and the recorded refutation was weak**): a warning appearing is content, not refresh chrome, and interpolation writes the DOM only when the sentence changes, so the strip announces once on arrival and never per tick — which is the accessible behavior, not a violation of `:580`. A failing banner endpoint logging once per tick: real for two of the four fault branches — `AdminPort.Fail` *does* log when the vendor status is an error, and `Invoke`'s catch logs — so the triage log's "`AdminPort.Fail` writes no log" is wrong; kept anyway, because the noise needs a genuinely erroring vendor endpoint and suppressing it would add a parameter to `Invoke`'s public surface. The banner's endpoint held to no privilege rule of its own: **verified rather than asserted** — `Task.Manager.ResourcesOR()` answers `%Admin_Task,%Admin_Operate`, identical to `Task.CRUD`, so this story's banner is genuinely covered by the read's own pair and a least-privileged operator does see the strip. AD-36's spine text describing a `rowGet` `type` no engine admits: the spine states target architecture, as it does for AD-43's six screens and AD-6, and DW-269 owns the work. `BannerCorpus.DeclarationFor`'s `unassigned` type hint: unreachable — every case carries the key; reopen if a case omits `banner`. `Banner/Bad.cls`'s `error` severity: the closed set lives in the registry, not the stylesheet, and adding `error` to it would fail loudly rather than silently. `assert.ok(inSys < total)` implied by the helper: redundant, not vacuous — the helper carries the real assertion and the line carries the readable message. `Test/Smoke.cls`'s unasserted `tOk`: the test's claim is the report line, and the sibling checks share the shape. The dropped every-row containment check in three specs: it asserted over the rendered window, which is the population `filterToSubset` deliberately stopped using. Three sibling specs keeping their own copy of the marker parsing: identical today; convert with the next spec that touches one. No browser-tier leg showing a *tick* raising the strip: pinned at the component tier with a real tick. The `[ASSUMPTION]` label on the refresh rates not travelling into the descriptor: shipping them ratifies the assumption, which is the lead's to record. `Suspended`-is-always-false as a universal: supported by the `RunList` coercion mechanism, which the descriptor states, not by the two probes alone. Archetype unchecked on a banner, the `.ocu-banner-*` stylesheet scan's brittleness, `viewCount`'s `-1`/`0` passed inline, and `irisSession` not checking `spawnSync`'s status (the transcript is already in the failure message): each guards state never demonstrated.
...
**QA pass (2026-09-14).** Reviewed the three candidates the epic cycle named and closed the two that were real gaps; DW-270 (the stopped-Task-Manager banner) is out of this pass's footprint, routed to the burn-down as directed.

- Banner fault path — reviewed, no new test. `ScreenRead:TestADeclaredBannerIsResolvedInsideTheReadAndNeverFailsIt` already drives a real fault (`Task.NoSuchManager` has no compiled class behind it, so `AdminPort.Invoke` genuinely refuses with `PORT.NOTIMPLEMENTED`, not a stub) and already makes the fault-suppressed case distinguishable from the correctly-absent case: the Faulting fixture's `equals` is `"Running"`, the same value the Matching fixture (against the real `Task.Manager` endpoint) proves the live instance answers moments earlier in the same method, so the Faulting fixture's `""` is known to be a suppression rather than a non-match. Sufficient as shipped.
- Auto-refresh "survives" assertions — real gap, closed. `tasks.browser-spec.mjs`'s AC4 leg puts a filter, a selection and a scroll offset away from their defaults before the tick, but never the sort (this screen ships no sort control), so a tick that silently reset the sort back to the descriptor's default would be invisible there: "before" already equals what "reset" would produce. `ui/tools/screen-store.test.mjs`'s new test closes it at the store tier, where all four can be set away from default directly.
- The poll after `SuspendSet` — real gap, closed without touching any instance. `setTaskManagerSuspended`'s marker-parsing was extracted, behavior-preserving, into `ui/browser/iris-session.mjs`'s `parseMarkers` (verified via a full throwaway run of `tasks.browser-spec.mjs`, 4/4 green, Task Manager left running, container torn down) and pinned in `ui/tools/iris-session.test.mjs` against transcripts shaped as a real refusal and a real non-convergence would leave them, proving each fails on the right assertion (the helper's own `SuspendSet(...) succeeded`, or the caller's `...is suspended`) rather than hanging or passing silently. A genuine vendor-level `SuspendSet` refusal has no known safe trigger short of weakening the throwaway's own security posture, so this is pinned at the parsing tier instead of live.

Mutations (Rule 19), each applied, observed red, reverted; `git status --short` and `git diff --stat` were byte-identical to the pre-mutation snapshot afterwards:

- Sort survives the tick — mutation: add `this.sortBy = '';` to `ScreenStore.applyTick` → `screen-store.test.mjs`'s new "a tick leaves sort, direction, filter, selection and scroll alone..." test red **and** `refresh.test.mjs`'s "a tick replaces data, truncated, banner and lastUpdate and nothing else", which already moved sort off default. `this.sortDirection = '';` instead reddens the new test alone: `direction` is the slot only it carries. Both observed at review.
- Marker parsing — mutation: change `parseMarkers`'s missing-marker default from `null` to `''` → `iris-session.test.mjs`'s "a marker the transcript never wrote parses as null" test red. Observed.
- Strip class list (added at review) — mutation: drop `ocu-list-page-banner` from `ListPage.bannerClass` → `list-page.spec.ts`'s banner test red on the class list, alone. Observed; `.ocu-banner` is `display: block`, so that class is what lays the glyph beside the sentence and separates the strip from the table, and no severity-only assertion sees its loss.
- Live Task Manager precondition (added at review) — `ScreenRead`'s banner test now asserts `TASKMGRStatus() = 1` rather than stating it in a comment; suspending an instance to redden it is the browser spec's job, on the throwaway, so the assertion is pinned by construction rather than by mutation.
- Refused `SuspendSet` / unconverged poll (added at review) — mutation: drop `taskManagerStateFrom`'s `assert.equal(values.OK, '1', ...)` → `iris-session.test.mjs`'s refusal test red, alone; answer the wanted state rather than `values.STATE` → its unconverged-poll and missing-marker tests red. Both observed; the pins now execute the decision `setTaskManagerSuspended` takes, which the two they replaced did not.

Files (QA): `ui/tools/iris-session.test.mjs` (new) · `ui/browser/iris-session.mjs` (new, `parseMarkers` extracted for testability) · `ui/tools/screen-store.test.mjs` (changed, one test added) · `ui/browser/tasks.browser-spec.mjs` (changed, `irisSession` delegates to `parseMarkers`; the four legs re-ran green on a fresh throwaway afterward).
