...
baseline_commit: 'a3cee61f04f8c74a9364b862813ad3416ece403a'
...
- 2026-09-21 (lead, resuming after the weekly limit): the three-tier browser-spec grant was given in full, so the second `blocked` is resolved and `status` is set `done`. `[CI]` `gates (node 26.0.0)` was red on `3fc688b` at `home.page.spec.ts`'s AD-43 row (`expected 4 to be +0`) and green on the other two bands: diagnosed as environment contamination, not a timer this story added -- `vi.getTimerCount()` is environment-global, and three real `Timeout` handles are pending when that row begins, which `advanceTimersByTimeAsync`'s yields to the real loop can let fire inside the measurement window. Fixed by draining the real clock before the fake one is installed; the assertion stays `toBe(0)` and AD-43's promise is unchanged. That first fix was under-sized: a fixed 100 ms drain held on node 26 and did not on node 24, where CI then read `expected 1 to be +0` where node 26 had read 4. A count that varies by band is contamination, not a timer the component registers. Replaced with a bounded wait for the real loop to be verifiably empty plus a **synchronous** ten-minute advance, which yields to the real loop not at all; the two assertions keep their reach, demonstrated by mutations A and B.

`[CI]` The Integration AC's AC2 clause also failed on CI while passing locally, and for the reason this project has a pitfall about: `carriesState` decided "none of the six is in browser storage" by searching the stored values for `IRIS`, `432`, `sideBar` and `panel.width`, and the values it searched are a JWT and a random per-tab nonce. A needle as short as `432` turns up in random material often enough to redden an otherwise green run. Replaced with the structural claim AC2 actually makes -- the `sessionStorage` key set holds nothing beyond `ocupilot.token-pair` and `ocupilot.tab-nonce` (`core/token-store.ts`, Story 1.6) -- which is deterministic and strictly stronger, because it catches a seventh key whatever its value contains.
...
- mutation (AD-43, A): HomePage registers one bare 30-minute `setTimeout` -> the AD-43 row went red on `expect(vi.getTimerCount()).toBe(0)` with `expected 1 to be +0`, so the row still catches a timer that issues no read.
- mutation (AD-43, B): HomePage joins the roster with `setInterval(read, 60_000)` -> the row went red on `expect(systemInfo.calls.length).toBe(issued)` with `expected 12 to be 2`, i.e. the interval fired ten times **under the synchronous advance**, which is what shows the advance keeps the reach the async one had. Both reverted, `home.page.ts` byte-identical.
...
- mutation (AC2, the storage clause, structural): `ShellState.setOpen` mirrors the open state back into `sessionStorage` -> `browser/ui-state-survives-sign-out.browser-spec.mjs` went red naming the leaked key, `sessionStorage holds only the per-tab token pair: [["ocupilot.side-bar.open","0"],...]`; reverted, rebuilt, redeployed, green again, `shell-state.ts` byte-identical.
