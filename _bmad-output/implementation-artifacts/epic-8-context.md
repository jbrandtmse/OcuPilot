...
Build step 5. A user creates the things the six areas administer (a web application, a user, a role with its resource grants, a resource, a device, a wallet secret, an X.509 credential) through medium forms that validate server-side and open the new entity on success. Every one of those writes is also reachable by the agent as a confirmed proposal. Story 8.1 (done) settled the create write kind and the `form-page` contract that the rest of this epic and Epic 9's editors inherit. At least one create or edit form per area from this step is part of the 2026-09-27 floor. Story 8.9 is the floor's last act, a verification rather than a form.
...
- **One form contract.** A full-page route under the locator bar: one column of at most 720px, fields of at most 480px (confirmed in 8.1 and pinned by a design-token test), fields in **the classic order**, Material outlined fields with the label above and helper text beneath, an asterisk plus a one-line legend for required fields, and a sticky 56px action bar with Save as the one primary and Cancel as a text button. Read the classic order from the classic page on the instance.
...
- **Privilege grants are refused on the instance whatever the caller, the screen included.** That covers adding `%All` or any `%Admin_*` role to a user or role, setting application roles on a web application, and a resource grant that escalates privilege. On a screen the server's refusal sentence lands on the field it concerns (the user's Roles field) as a `{field, code, reason}` violation. A picker may pre-mark such roles unavailable, but the instance refusal is the prohibition. These grants stay a classic-portal action in Release 1.
- **No caller-supplied filesystem path, including on a create.** A WSGI/ASGI web application's location is a relative directory name only. It is resolved at call time under one fixed root found on the instance, never cached, with a literal `..` rejected and a strict name pattern enforced, on both the screen route and the agent tool. The form shows the resolved path read-only, and arbitrary locations stay a classic-portal action. Story 8.2 fixes 8.1's create, and the fix blocks the floor.
- **An unauthenticated web application is allowed but named.** The agent's create proposal states the unauthenticated consequence, and the create form states it at the authentication-method field, because the screen has no confirmation dialog.
...
- **Create is a third write kind (AD-54), beside merge and action-style.** A tool declares that it creates its target. The fresh read inverts: a 404 is the precondition, and a present target refuses the mint. The fingerprint covers the target's **absence** under the entity type's canonical spelling, and confirm refuses if the name was taken after the mint. The payload is composed from the arguments over the derived field list, so every supplied field is a diff row and the unchanged count is zero. Every other gate is unchanged. The absence fingerprint is required even where the vendor refuses duplicates, because `WebApp.App`'s PUT is a silent upsert.
- **The screen's Save and the agent's confirm are two callers of one tool class (AD-55).** The route takes endpoint, request type, settable fields, payload composition and port from the tool. It evaluates the prohibited set before any port is touched and inherits the write's validation and fingerprint gates. It mints no proposal and takes no confirm token. **Enforced read-only and the kill switch do not gate a screen Save**: they belong to the agent, and every screen keeps working with the agent disabled. A screen that composes its own payload is a review failure. The one exception is a self-service action on the user's own account.
...
- **Field lists are derived at build time and committed.** Each field is classified `ordinary`, `secret` or `opaque`, and an unclassified field is emitted secret. 8.5 ships the first non-empty secret-argument declaration (`X509Credential`'s `PrivateKeyPassword`).
- **`Wallet.Secret` publishes no body template.** Its body is `{Type, WalletSecretConfig}`, with one list per `Type`, derived from the underlying classes and pinned by a test that fails when the instance disagrees. Its PUT is an upsert that the fresh read and fingerprint guard, with a test. Settle `Usage` and `Secret` on the instance before planning the payload, because the class rows and the published spec disagree.
...
- **Settle the wire on the instance before guessing.** 8.1 found `Type` and `Name` refused in the body and some enumerations sent as display strings.
...
- **Both earlier open questions are decided.** Read-only and the kill switch do not bar a person's own Save. `WSGIAppLocation` gets no exception and is contained under a fixed root. Story 8.2 carries the containment fix to 8.1's create, and a test must show that `..`, an absolute path and a sibling escape are each refused through both callers.
