...
- **Assume nothing merges.** `Security.User` and `Security.X509Credential` erase omitted fields; `Security.Resource` and `Wallet.Secret` keep them, and a `Wallet.Secret` PUT carrying a value creates an absent name (AD-4). Every edit reads fresh and sends the complete property set regardless. `security.x509.update` sends exactly `{OwnerList, CAFile, PeerNames}` from a fresh read; certificate, key and password travel only in the import.
