...
- [ ] DW-1699 (harvested from B8): `Api/Context.cls:50` passes `.tStage` to `RenderBadBody`, so a read or decode fault on `PUT /agent/context` answers 500 like the six sites DW-1165 fixed. Test: a `Test/DefinitionsFaults.cls` case.
...
**This pass: Batches B4 and B8** (baseline `264c87315cf1e98f4984867302f5038715ec4850`); all 12 items landed and are ticked. B2+B3's result is superseded; its triage log stands above.

- **B4:** the walk declares `OVERFLOW_ALLOWANCES` (the resize handle's 4px, pinned equal to its `left: -4px`) and visits inline `svg` roots (DW-1583, DW-1588: no fresh svg key, so its fallback does not apply). Appended to `_components.scss`: the status bar's right group stops shrinking and the left clips (DW-1584); one 24px checkbox/radio rule, which needed `flex-shrink: 0` beyond the task text to clear four audit and task-history keys (DW-1587). All 223 baseline entries were these three findings, so `entries` is empty. A Process details pause leg at the registry's route `processes/details/<pid>` (DW-1471).
- **B8:** `HandleTest` captures the version at the open and `ConnectionOutcome` takes it; an empty version is refused only on the as-stored branch (DW-426, DW-427, DW-1286; a fifth caller, `Test/AgentConnectionBound.cls`, passes its version). A read or decode fault answers 500 at the six sites (DW-1165, with `Dispatch.InvokeHandlerWithRequest`). `RenderForbidden` goes through `Kernel.Denial` (DW-1304); `Account.RenderViolations` delegates with its code (DW-1294). Both PUTs take an optional JSON-number `rowVersion`; both GETs and PUT answers carry it; the switches GET reads it before `Resolve`; both stores keep it and send it (DW-425). Beyond the task text, `DefinitionForm.retakeVersion` re-reads after the screen's own key store or Test connection, which write the row without projecting it, and takes the new version only when every writable field reads what those writes leave; otherwise a Save after Test connection over unsaved edits would be refused over a row only this screen wrote.
- Files: `Api/{Definitions,Switches,Account}.cls`; tests `Test/{AgentConnection,AgentConnectionBound,AgentConnectionRoles,AgentWire,AgentWireSecurity,DefinitionsFaults,Dispatch,RouterFixture,SwitchesStaleProbe,SwitchesWire}.cls`; `ui/browser/{structural-walk.mjs,structural-baseline.json,a11y-structural-invariants.browser-spec.mjs,process-control.browser-spec.mjs}`; `ui/tools/structural-baseline.test.mjs`; `ui/src/app/areas/agent/{definition-form,switches}.{store,page.spec}.ts`; `ui/src/styles/_components.scss` (appended at the end; Epic 16 also appends there, so expect a tail merge).

**Review:** 17 findings (medium 4, low 10, false 3). Patched: 3 medium entries (DW-1165's four untested sites, the unpinned refused-test re-take, the re-take adopting another writer's `enabled`) and 3 low (DW-1286's mutation, the Process details resume wait, a wrong DESIGN.md citation). Deferred: 1 low entry (`Api/Context.cls` passes no stage). Rejected: 3 false and 4 low, each with its reason in the triage log. Follow-up review: `true`. The re-take rule is not in the task text and is pinned only by jsdom specs against mocked answers; no browser leg drives key store, Test connection and Save on the instance.

**Verification** (`ocupilot-b-ci`, `LoadDir` 0 errors, source equal to the tree; one class per call, confirmed in `%UnitTest_Result`): `AgentConnection` 20/20 (run 76), `DefinitionsFaults` 3/3 (73), `SwitchesWire` 22/22 (67), `AgentWire` 18/18 (68), `AgentWireSecurity` 4/4 (69), `AccountPasswordWire` 4/4 (70), `SwitchState` 13/13 (71), `AgentConnectionBound` 15/15 (55). `npm run test:components` 1455/1455; `npm run test:tools` 1475/1475; browser specs on the redeployed bundle: `definitions` 9/9, `process-control` 3/3; `a11y-structural-invariants` 12/12 on the implement bundle, whose stylesheet is byte-identical to the final one. `check-objectscript.py` 0 problems. **Client initial bundle total: 1.84 MB** (1,842,353 bytes at the implement build), under 1854kB. EXPERIENCE.md, `strings.ts` and `Error.cls` untouched. Every behavior fix has its `mutation:` line in the B4/B8 blocks. Matrix Test Audit: the DW-425 row is covered by `SwitchesWire.TestASaveCarryingAStaleVersionIsRefusedUntilReloaded` and `AgentWire.TestAnUpdateCarryingAStaleVersionIsRefusedUntilReloaded` (two stale saves 409, the other writer's value survives, reload then 200), both green. Contended Epic 16 file: `ui/src/styles/_components.scss` only.

**Residual risk:** Switches now refuses a save when the row moved for any reason while the screen was open, including the auditing "writes marked" flag another screen sets on the same row; the conventions call that correct, and it is new behavior. `AgentConnection` (882 lines), `AgentWire` and `SwitchesWire` were already over the ~500-line guideline and grew.
