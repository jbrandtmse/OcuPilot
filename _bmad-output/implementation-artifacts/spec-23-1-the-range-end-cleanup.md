...
created: '2026-09-26'
status: 'done'
baseline_revision: '6e59892a9415d9104ec1809d0162aef39ee0c02b'
baseline_commit: '6e59892a9415d9104ec1809d0162aef39ee0c02b'
...
deferred:
  ...
  - summary: >-
    ...
    severity: low
  - summary: >-
      Api/Context.cls HandleUpdate calls Switches.RenderBadBody with no stage, so a read or decode fault on PUT /agent/context still answers 400 AGENT.BADBODY.
    evidence: |-
      Context.cls:50 passes only tReadSC; the six sites DW-1165 named now pass tStage and answer 500 for read/decode. Pre-existing; the fix is one argument plus a DefinitionsFaults case.
    location: >-
      src/OcuPilot/Api/Context.cls:50
    severity: low
  - summary: >-
      Api/Error.cls says "thirteen PROVIDER.* codes" at :205 and "thirteen PROVIDER.* sentences" at :1057; with PROVIDER.TOOLCALLFAILED appended there are fourteen.
    evidence: |-
      Counted from the class's Parameter PROVIDER... lines and Base.ReasonFor's arms. The two-word fix is mid-file, and B6 may only append to Error.cls (shared-append; Story 11.11 edits the same file).
    location: >-
      src/OcuPilot/Api/Error.cls:205, src/OcuPilot/Api/Error.cls:1057
    severity: low
---
...
- [x] DW-1105: add a new leg. It stores proxy `127.0.0.1` through `State.Egress.SetGuarded`, runs a turn on the fixture definition, asserts that progress answers `PROVIDER.EGRESS` with no credential, and restores the prior Egress row. Replace the header's "no turn can carry an egress refusal". File: `Test/TurnProviderFault.cls`.
- [x] DW-1217: append `100.100.100.200` to `METADATAADDRESSES` (`Kernel/Egress.cls:71`). Add `https://100.100.100.200/latest/meta-data` to `Test/Egress.cls` `TestACloudMetadataAddressIsRefusedInEveryFamily`; it is refused even when marked local.
- [x] DW-1222: reword the three comments without a count ("every property SecurityFields names"). Add an `ApplyChange(tA,"HttpAcknowledged",1)` leg after ReEnable, asserting flags `"0|0"`. Files: `Test/DefinitionsFieldGapProbe.cls:5-6`, `Test/AgentSchema.cls:192`, `Test/AgentState.cls:300-304,336`.
- [x] DW-1661: map `MALFORMED_FUNCTION_CALL`, `UNEXPECTED_TOOL_CALL` and `TOO_MANY_TOOL_CALLS` to a new MessageAdapter stop reason. `Loop` routes it to a new appended code, `PROVIDER.TOOLCALLFAILED`, with the reason "The model could not make a valid tool call. Try again, or rephrase the request." Rewrite the `GEMINISTOPREASONS` doc. Files: `Kernel/Provider/MessageAdapter.cls:67-75`, `Kernel/Agent/Loop.cls:225`, `Api/Error.cls` (append), `Provider/Base.cls` `ReasonFor`. Tests: `Test/Adapter.cls` (the `:609` case list; counts at `:1053-1058`), plus a `Test/TurnStream.cls` leg.
- [x] DW-1172: `ArgumentsTruncated = ''pArgumentsTruncated || ($Length(pArguments) > +$Parameter(pLimitsClass,"LEDGERROWMAXLENGTH"))`. File: `Kernel/State/Ledger.cls:251-252`. Add a new `Test/LedgerWire.cls` method under `Test.LedgerLimits`.
- [x] DW-1383: delete `ProviderPort.GateAnyOf` and `Screen.Gate.EvaluateAnyOf`, which have no callers. `GateAnyOfForUser`'s doc states the OR-set semantics. Reword the mutation text at `Test/ProviderPortOwner.cls:62-66` and `Test/PortGate.cls:137-139`. Compiling the package proves nothing called them.
- [x] DW-1650 (decided at the 10.6 merge gate): the one transport retry in `Provider/Base.cls` `Attempts` excludes a connect that never opened -- `ERROR #6059` with no `HttpResponse` -- so a refused or unreachable endpoint fails after one attempt. Read the failure code from the `%Status` (measure the shape on `ocupilot-b-ci`, e.g. a Post to `http://127.0.0.1:1/` with Timeout 3). A broken connection after the request was sent keeps its one retry. Test: the existing transport-retry test class (`Test/ProviderTransportRetry.cls` or its equivalent) gains a connect-refused leg asserting one attempt. The lead amends AD-42 in L.
- [x] DW-1699 (harvested from B8): `Api/Context.cls:50` passes `.tStage` to `RenderBadBody`, so a read or decode fault on `PUT /agent/context` answers 500 like the six sites DW-1165 fixed. Test: a `Test/DefinitionsFaults.cls` case.
- [x] DW-1322: after DW-1383, an OR-set refusal carries no `failedPair` detail, and the doc says so. Files: `Port/ProviderPort.cls:489-512`, `Test/ProviderPortOwner.cls:90`.
...
- [ ] DW-1706 (harvested from B6 review): `Api/Error.cls`'s `PROVIDERTRANSPORT` doc comment says the failure is never retried; replace that sentence in place with AD-42's rule (retried once on a new connection, except a connect that never opened). Comment only, same line count.
...
- [x] DW-426: `HandleTest` captures `+tRow.RowVersion` beside `ValuesFromRow` and passes it to `ConnectionOutcome` as a new parameter. `ConnectionOutcome` no longer calls `GuardedVersion`, and its four test callers pass the version they read. Files: `Api/Definitions.cls:795,862,880-889`, `Test/AgentConnection.cls:149,744`, `Test/DefinitionsFaults.cls:178,187`. Restate the comments at `:854-861` and `:876-879` as "captured at the open that read the stored values".
- [x] DW-427: refuse an empty version only on the as-stored branch, before the call. File: `Api/Definitions.cls:880-889,919-921`. In `Test/DefinitionsFaults.cls`, the as-stored call with `""` answers 500 with 0 provider calls, and a not-as-stored leg makes the call and answers 200.
- [x] DW-1286: pass the RowVersion read at `:733` into `ConnectionOutcome`, and assert `ProviderStub.Calls()=1` and the reason "The provider answered and the agent definition could not be marked verified". File: `Test/AgentConnection.cls:731-751`.
- [x] DW-1165: pass `.tStage` at the six `ReadRequestBody` call sites. `RenderBadBody` takes an optional `pStage` and renders `RenderInternal` (500) for `read` or `decode`, and the 400 otherwise. Files: `Api/Definitions.cls:374,439,625,786,1349`, `Api/Switches.cls:110,185,450`, plus a `Test/Dispatch.cls` variant that accepts a prebuilt `BodyRequest` with `FaultOnContent=1`. Test: `Test/DefinitionsFaults.cls`.
- [x] DW-1304: `RenderForbidden` returns `##class(OcuPilot.Kernel.Denial).Envelope(ADMINRESOURCE_":"_ADMINPERMISSION)`, and its doc becomes one line. Files: `Api/Definitions.cls:1323-1333`, `Test/AgentWireSecurity.cls:232,300` (assert the reason), and the fixture text at `switches.page.spec.ts:47`.
- [x] DW-1294: `Definitions.RenderViolations` takes `pCode As %String = AGENTVALIDATION`, and `Account.RenderViolations` delegates to it with its own code. Files: `Api/Definitions.cls:1386`, `Api/Account.cls:168-180` (its doc says it delegates "with this route's code").
- [x] DW-425: the switches GET reads the version before `Resolve`. Both PUTs take an optional JSON-number `rowVersion` and pass it as the expected version, and a body without it behaves as today. `definition-form.store.ts` and `switches.store.ts` keep the version from the load and from each 200, and send it on save. Replace "no version token crosses the wire" at `Switches.cls:119-124` and `SwitchesStaleProbe.cls:14-16` with "the version is the one the screen read, sent back on save".
...
### 2026-09-26 — Review pass (Batches B4+B8)

- verdicts: 17 findings — high 0, medium 4, low 10, false 3, maybe-false 0
- findings:
  - `[medium]` `[patch]` verification-gap: DW-1165's read-fault leg drove two of the six sites and no `decode` stage — the loop now drives all six; the four-site mutation reddens each (run 74). A decode fault has no seam; it takes the same `If` as `read`.
  - `[low]` `[defer]` verification-gap: `Api/Context.cls:50` calls `Switches.RenderBadBody` with no stage, so a read or decode fault on `PUT /agent/context` still answers 400 — pre-existing, and the task names six sites; in `deferred:`.
  - `[low]` `[patch]` verification-gap: DW-1286's recorded mutation left its two new assertions unexercised — restoring the pre-call version refusal reddens exactly those two (run 75); line added.
  - `[low]` `[patch]` verification-gap: the Process details leg's resume wait passed with the chip gone or reading "off" — it now requires the chip present and reading the running form.
  - `[medium]` `[patch]` verification-gap: the re-take after a refused Test connection was unpinned — new component test (key store, refused test, Save sends the re-read version; a row edited elsewhere keeps the old one); mutation red.
  - `[low]` `[reject]` verification-gap: `HandleTest`'s capture point is unpinned below `ConnectionOutcome` — a regression needs the statement moved, and pinning it needs a probe subclass with an in-request writer.
  - `[low]` `[defer]` verification-gap: `Api/Context.cls:50` is a defect as well as a gap (same root cause as the second row) — same deferred item.
  - `[low]` `[reject]` intent-alignment: no browser leg joins DW-425's halves — the server legs run over a real socket and assert the JSON-number type, the client specs assert the number sent; a joining leg is more than a direct correction.
  - `[medium]` `[patch]` intent-alignment: `retakeVersion` skipped `enabled`, so another writer's enable/disable between load and Test connection was adopted and overwritten by the next Save — `enabled` is now compared exactly (false after this screen's key store, which disables the definition); the `run({ enabled: true })` leg pins it. Server-owned fields are not in the save body, so adopting their changes overwrites nothing.
  - `[medium]` `[patch]` intent-alignment: DW-1165 tested at two of six sites (same root cause as the first row) — same patch.
  - `[low]` `[defer]` intent-alignment: `Api/Context.cls` keeps its 400 (same root cause as the second row) — same deferred item.
  - `[low]` `[reject]` intent-alignment: DW-426/427 tests run below the route, and the as-stored empty-version refusal is unreachable from `HandleTest` — the task defines that guard for direct callers; the capture point is the sixth row.
  - `[false]` `[reject]` intent-alignment: the walk cannot see what the status bar's left group clips — clipping that tail is the specified behavior, and the walk exempts clipped content by design.
  - `[low]` `[patch]` intent-alignment: the new status-bar comment cited "DESIGN.md's Yield order", which governs the side bar, panel and content row — now cites DESIGN.md `status-bar`.
  - `[false]` `[reject]` intent-alignment: DW-1587's `appearance: none` check was not recorded — checked: no template holding those labels uses either switch class; the check holds.
  - `[low]` `[reject]` intent-alignment: no bundle total in the Results lines — the fix edits the spec; the total is in `## Auto Run Result`.
  - `[false]` `[reject]` intent-alignment: the pass changed the spec's `status` and baselines — those are the workflow's own step-03/04 writes.

### 2026-09-26 — Review pass (Batch B6)

- verdicts: 9 findings — high 0, medium 1, low 7, false 1, maybe-false 0
- findings:
  - `[low]` `[reject]` verification-gap: `TurnProviderFault`'s restore of an existing Egress row never runs (the throwaway has none), and a failed snapshot query reads as "no row" — throwaway-only test support; a leaked proxy reddens the next method loudly; the fix adds branches.
  - `[low]` `[reject]` verification-gap: DW-1383 has no `mutation:` line, and the two reworded doc mutation texts (`ProviderPortOwner`, `PortGate`) were not observed — dead-code deletion is proved by the package compile, the compiled-method check and the grep, as the spec directs; the reviewer confirmed both reworded claims by reading.
  - `[low]` `[defer]` verification-gap: `Api/Error.cls:205` and `:1057` still say "thirteen `PROVIDER.*`", and there are fourteen — the two-word fix is mid-file in a file this batch may only append to; in `deferred:`.
  - `[low]` `[reject]` verification-gap: the new leg restores the Egress row inline, so a throw between store and restore leaves a loopback proxy — assertions do not throw, the class runs on a throwaway only, and a guaranteed restore adds structure.
  - `[medium]` `[patch]` intent-alignment: DW-1699's ledger trailer and the B4+B8 review route `Api/Account.cls:57` to B6 too, and a read or decode fault there still answered 422 — `RenderBodyRefusal` now answers 500 `INTERNAL` for those stages, its composed status the logged detail (AD-35 kept); `DefinitionsFaults` gains the case; mutation red (run 114), reverted.
  - `[low]` `[reject]` intent-alignment: DW-1661 is pinned in two halves (`Adapter`'s mapping, `TurnStream` on the Anthropic stub) and no Gemini body runs through a turn — both halves read the one constant `TOOLCALLFAILEDSTOPREASON`, and the streamed Gemini reply maps through the same `GeminiToCanonical` (`MessageAdapter.cls:470`); a Gemini turn leg is more than a direct correction.
  - `[low]` `[reject]` intent-alignment: DW-1650 is pinned at the adapter, not in a turn, and the halved time is not measured — a turn's provider step calls the same `Attempts`, and the time follows from one attempt of the measured 3.03 s connect; a turn-level timing leg is more than a direct correction.
  - `[false]` `[reject]` intent-alignment: AD-42's DW-1650 amendment and its Deferred row are not in the diff — not a defect of this pass: `:228` assigns that edit to the lead in Batch L, and the implement pass may not edit the spine.
  - `[low]` `[defer]` intent-alignment: the same `Error.cls` count (same root cause as the third row) — same deferred item.
...
- Results (implement pass, 2026-09-26, `ocupilot-b-ci`; bundle rebuilt, copied and the installer re-run before every browser read): `a11y-structural-invariants.browser-spec.mjs` 12/12, the walk finding 0 entries against an emptied baseline (all 223 entries were DW-1583/1584/1587); `process-control.browser-spec.mjs` 3/3; `npm run test:tools` 1475/1475. DW-1587 needed `flex-shrink: 0` on the shared input rule: without it the walk found 4 fresh keys (the audit and task-history criteria checkboxes at 17-19px, shrunk by their long labels). DW-1588 found no fresh svg key, so its fallback does not apply. The Process details route is `os-management/processes/details/<pid>` (the registry's), not `processes/<pid>`.
- mutation: `OVERFLOW_ALLOWANCES` emptied → the walk reported the resize handle 4px past `aside.ocu-panel` on every screen at 1280 and 720 (AC5 red), and `structural-baseline.test.mjs` "the resize handle's overflow allowance is the hit area's own offset" red; reverted.
- mutation: the `.ocu-status-bar-group:last-child { flex-shrink: 0; }` rule deleted → 91 fresh overflow keys at 720 (`ocu-status-bar-connection` ×58, `ocu-status-bar-stamp` ×33), AC5 red; reverted.
- mutation: the shared checkbox/radio sizing rule deleted → 16 fresh `min-width` keys (eight screens' inputs at 13px, 1280 and 720), AC5 red; with only its `flex-shrink: 0` removed → 4 fresh keys (audit, task history); reverted.
- mutation: the walk's filter narrowed back to `HTMLElement` → the new "an inline svg wider than its non-clipping parent" liveness leg red; `width: 40px` added to the Story 15.7 area-icon svg rule (widened filter) → 2 fresh keys, `app-home-page>svg` 16px past `span.ocu-area-tile-icon` at 1280 and 720, AC5 red; reverted.
- mutation: `onBusEvent` in `ui/src/app/core/refresh.ts` returns on `proposal-open` for a screen with a `parentScope` → the Process details leg red, the list leg and AC1 green; reverted.
...
- Measured (DW-1650, `ocupilot-b-ci`, 2026-09-26): `%Net.HttpRequest` `Post("http://127.0.0.1:1/")` with `Timeout` 3 and `SocketTimeout` 0 returned after 3.03 s with `ERROR #6059: Unable to open TCP/IP socket to server 127.0.0.1:1` (`GetErrorCodes` 6059, `Equals(sc,6059)` 1), and `HttpResponse` was not an object. Read in `irislib/%Net/HttpRequest.cls`: both `$$$HttpRequestConnect` sites (the socket open, `:1679`, and the proxy `CONNECT` reply, `:1257`) come before the request is written, and `HttpResponse` is created only after it (`:1498`). `Attempts` tests `$System.Status.Equals(tIssueSC, $$$ERRORCODE($$$HttpRequestConnect))`.
- Results (implement pass, 2026-09-26, `ocupilot-b-ci`, `LoadDir` 0 errors, then `CompilePackage("OcuPilot")` 0 errors; each class alone, confirmed in `%UnitTest_Result`): `TurnProviderFault` 6/6 (run 111), `Egress` 23/23 (104), `AgentState` 13/13 (105), `AgentSchema` 8/8 (86), `Adapter` 25/25 (110), `TurnStream` 6/6 (112), `LedgerWire` 6/6 (106), `ProviderPortOwner` 4/4 (107), `PortGate` 4/4 (91), `Envelope` 15/15 (92), `ProviderTransportRetry` 9/9 (108), `DefinitionsFaults` 3/3 (109), and `ProviderRetry` 12/12 (94) for the `Attempts` change. `check-objectscript.py` 0 problems. DW-1383: after the package compile, `%Dictionary.CompiledMethod` holds neither `ProviderPort||GateAnyOf` nor `Gate||EvaluateAnyOf`, and `grep -rnE 'EvaluateAnyOf|GateAnyOf([^F]|$)' src ui/src ui/tools ui/browser scripts` prints nothing.
- mutation: `100.100.100.200` removed from `METADATAADDRESSES` → `Egress.TestACloudMetadataAddressIsRefusedInEveryFamily` red on that address's three assertions alone (run 95); reverted.
- mutation: `HttpAcknowledged` removed from `Agent.SecurityFields` → `AgentState.TestASecurityChangeDisablesAndAModelChangeDoesNot` red on the new leg alone (run 96); reverted.
- mutation: `Ledger.GuardedAppend` sets `ArgumentsTruncated` from `pArgumentsTruncated` alone → `LedgerWire.TestAnArgumentStringTheStoreCutsIsMarkedTruncated` red on the long case (run 97); reverted.
- mutation (DW-1322): `GateAnyOfForUser` sets the set's first pair on a refusal → `ProviderPortOwner.TestTheGateReadsTheTurnOwnersLiveGrants` red on the no-detail assertion (run 98); reverted.
- mutation (DW-1650): `'tNeverOpened` dropped from the retry condition → `ProviderTransportRetry.TestAConnectThatNeverOpenedIsNotRetried` red, the transport entered twice and the queued answer mapped (run 99); reverted.
- mutation (DW-1699): the stage argument dropped at `Context.HandleUpdate`'s `RenderBadBody` call → `DefinitionsFaults.TestAnUnreadableBodyIsAServerFaultAndAMalformedOneIsTheClients` red on the context route, answering 400 `AGENT.BADBODY` (run 100); reverted.
- mutation (DW-1661): `MALFORMED_FUNCTION_CALL` mapped back to `refusal` → `Adapter` red in the case list and both counts (run 101); reverted. The seven mutations above ran in one load, each read on its own class and attributed by its failing assertion.
- mutation (DW-1105): the proxy refusal deleted from `ProviderPort.Dispatch` → `TurnProviderFault.TestAStoredLoopbackProxyFailsTheTurnOnEgress` red, the turn completing (run 102); reverted.
...
- Results (implement pass, 2026-09-26, `ocupilot-b-ci` after a `LoadDir` of `src/` with 0 errors; each class alone, confirmed in `%UnitTest_Result`): `AgentConnection` 20/20 (run 65), `DefinitionsFaults` 3/3 (66), `SwitchesWire` 22/22 (67), `AgentWire` 18/18 (68), `AgentWireSecurity` 4/4 (69), `AccountPasswordWire` 4/4 (70), `SwitchState` 13/13 (54); also `AgentConnectionBound` 15/15 (55), whose driver is a fifth `ConnectionOutcome` caller and now passes its version, and `ConnectionKey` 5/5 (56). `npm run test:components` 1454/1454; `npm run test:tools` 1475/1475; `definitions.browser-spec.mjs` 9/9 on the rebuilt bundle; `check-objectscript.py` 0 problems.
- mutation: `ConnectionOutcome` re-reads the version with `GuardedVersion` → `AgentConnection.TestTheVerificationIsConditionalOnTheVersionReadAtTheOpen` red (run 57); reverted.
- mutation: `If tTestedAsStored && (pVersion = "")` made `If (pVersion = "")` → `DefinitionsFaults.TestAnAsStoredTestWithNoVersionRefusesBeforeTheProviderIsCalled` red on the edited-values leg (run 59); reverted.
- mutation: the `tWriteSC` failure branch made `If 0 && ...` → `AgentConnection.TestTheVerificationWriteFailingAnswersFiveHundredAndNoAnswerBody` red (run 58); reverted.
- mutation: `.tStage` dropped at `HandleCreate`'s `ReadRequestBody` → `DefinitionsFaults.TestAnUnreadableBodyIsAServerFaultAndAMalformedOneIsTheClients` red, the read fault answering 400 (run 59, same run as DW-427's; separate methods); reverted.
- mutation: `RenderForbidden` restored to its literal sentence → `AgentWireSecurity` red on both refusal sweeps (run 60); reverted.
- mutation: the code dropped from `Account.RenderViolations`' delegation → `AccountPasswordWire` red on both envelope-code assertions (run 61); reverted.
- mutation: both handlers ignore the body's `rowVersion` → `SwitchesWire.TestASaveCarryingAStaleVersionIsRefusedUntilReloaded` red (run 63) and `AgentWire.TestAnUpdateCarryingAStaleVersionIsRefusedUntilReloaded` red (run 64); reverted. (Run 62 is void: the first attempt at this mutation did not compile.)
- mutation (client): `rowVersion` dropped from both stores' `body()` → the two "a save sends the row version the screen read" specs red; the field comparison in `DefinitionForm.retakeVersion` disabled → "after this screen's own Test connection over unsaved edits" red; reverted.
...
**This pass: Batch B6** (baseline `6e59892a9415d9104ec1809d0162aef39ee0c02b`); all nine items landed and are ticked. B4+B8's result is superseded; its triage log stands above.

- **Provider and egress:** a stored loopback proxy fails a turn `PROVIDER.EGRESS` through a real job, and the `TurnProviderFault` header says all five codes reach a turn (DW-1105). `100.100.100.200` joins `METADATAADDRESSES` (DW-1217). Gemini's three tool-call finish reasons map to a new canonical `tool_call_failed`, which `Loop.Run` ends as `PROVIDER.TOOLCALLFAILED`, appended at the end of `Error.cls`, with the task's sentence in `Base.ReasonFor` (DW-1661). `Attempts` no longer retries `ERROR #6059`, a connect that never opened (DW-1650).
- **Measured DW-1650 shape** (`ocupilot-b-ci`): a `%Net.HttpRequest` Post to `http://127.0.0.1:1/` with `Timeout` 3 returned after 3.03 s with `ERROR #6059: Unable to open TCP/IP socket to server 127.0.0.1:1` (`GetErrorCodes` 6059), and `HttpResponse` was not an object; `Attempts` tests `$System.Status.Equals(tIssueSC, $$$ERRORCODE($$$HttpRequestConnect))`.
- **Ledger, gate, handlers, tests:** the store sets `ArgumentsTruncated` when it cuts (DW-1172); `ProviderPort.GateAnyOf` and `Screen.Gate.EvaluateAnyOf` are deleted, and `GateAnyOfForUser` names no failed pair (DW-1383, DW-1322); a read or decode fault answers 500 on `PUT /agent/context` and, after review, on `POST /account/password` (DW-1699, both sites its ledger trailer names); three comments drop the stale count, and `AgentState` gains the `HttpAcknowledged` leg (DW-1222). Beyond the task text: `ProviderStub.QueueConnectRefused`, tied to the measured shape by `TestARefusedConnectAnswersNoStatus` asserting #6059 on the real transport.
- **Files:** `Api/{Account,Context,Error}.cls` (Error: one parameter appended at the end), `Kernel/{Egress,Agent/Loop,Provider/Base,Provider/MessageAdapter,State/Ledger}.cls`, `Port/ProviderPort.cls`, `Screen/Gate.cls`; tests `Test/{Adapter,AgentSchema,AgentState,DefinitionsFaults,DefinitionsFieldGapProbe,Egress,LedgerWire,PortGate,ProviderPortOwner,ProviderStub,ProviderTransportRetry,TurnProviderFault,TurnStream}.cls`. Contended with Story 11.11: `Loop.cls` (one hunk at `Run` :225, away from its `AnswerClientCall` hunks) and `Error.cls` (append only).

**Review:** 9 findings (medium 1, low 7, false 1). Patched: 1 medium (DW-1699's `Account.cls:57` site). Deferred: 1 low entry, two rows (`Error.cls`'s "thirteen" count, mid-file in an append-only file). Rejected: 5 low and 1 false, each with its reason in the triage log. Follow-up review: `false` (one medium patched).

**Verification** (`ocupilot-b-ci`, source equal to the tree, `CompilePackage("OcuPilot")` 0 errors; one class per call, confirmed in `%UnitTest_Result`): `TurnProviderFault` 6/6 (run 111), `Egress` 23/23 (104), `AgentState` 13/13 (105), `AgentSchema` 8/8 (86), `Adapter` 25/25 (110), `TurnStream` 6/6 (112), `LedgerWire` 6/6 (106), `ProviderPortOwner` 4/4 (107), `PortGate` 4/4 (91), `Envelope` 15/15 (92), `ProviderTransportRetry` 9/9 (108), `ProviderRetry` 12/12 (94), `DefinitionsFaults` 3/3 (115, after the review patch), `AccountPasswordWire` 4/4 (116). Ten `mutation:` lines in the B6 block, each observed red and reverted byte-identical. `check-objectscript.py` 0 problems. EXPERIENCE.md 981 lines, untouched; no client file changed, so no bundle build. Matrix Test Audit: no matrix row belongs to B6.

**Residual risk:** no leg runs a Gemini body through a turn; the mapping and the `Loop` branch are pinned separately and meet at one constant. DW-1650's halved error time is inferred from one attempt of the measured connect, not timed in a turn. The lead's Batch L half of DW-1650 (AD-42 and its Deferred row) is still open.
