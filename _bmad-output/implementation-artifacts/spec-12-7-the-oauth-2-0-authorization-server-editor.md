...
baseline_commit: '5e9c4d7d15a679e05be6e09e1438c6b6394e73d2'
...
Code review 2026-09-25, rework 1 re-review (four layers, full-opus; diff `5e9c4d7d..dd342b55`). Both `[CI]` items fixed at the root: `check`/`fill` reach the control by focus (Space toggles), the `waitForFunction` unchanged; the Sign out read waits two frames, which lets the change detection queued by the last keystroke (zoneless, `setTimeout`/rAF race) run before the read -- a scheduler condition, not the list itself, and a stale read can only answer `active: false`, never a false green. Sign out mutation re-demonstrated on a rebuilt, redeployed bundle (`## Verification`). 0 high, 0 medium, 3 low patched, 14 rejected.

- [x] [Review][Patch] (low) The Sign out test's in-file mutation note omitted the `active` assertion's mutation [`ui/browser/account-and-filter.browser-spec.mjs`:200]
- [x] [Review][Patch] (low) Rework 1's `## Verification` lines stated CI's layout and CI's keystroke interleaving as observed -- now `(inference)` [spec `## Verification`]
- [x] [Review][Patch] (low) Stale `check` :179 line reference in the same line [spec `## Verification`]
- Rejected: the diagnosis leaves the 2/2 CI onset unexplained (false: the full-query list holds Sign out alone, and the wait closes any timing cause; the onset is labeled inference); `fill`'s change unproven (false: AC1/AC2 read every filled value back, 5/5); `fill` on a `<select>` (false: no call site passes one); removed pointer-path coverage (false: Puppeteer's click never hit-tested); no focus-landed check in `check`/`fill` (low: `openTab` waits for the tab body shown, and a miss surfaces as `check`'s own timeout); `definitions`' stronger settle pattern not reused, five copies of the frames helper, two helper styles, no guard against a revert to `page.click` (low, no failure it would catch that this one lets through); stale focus/pointer claims in `users-editor` and `_components.scss` (low, outside the rework range); Auto Run Result's unlabeled inference, its "about 33 specs" count, its self-contradicting reproduction sentence and repetition of `## Verification`, and a `[false]` triage label that means out-of-scope (fix edits the spec outside `## Verification`); the cycle log's `repro=...CI_conditions` token (lead's log; the Verification lines now say what was reproduced).
...
- Rework 1, `[CI]` item 1: the grant checkbox lay in the form's scroll port under the sticky form bar on CI (inference; reproduced locally only at 1280x945), where Puppeteer's click does not scroll and lands on the bar; `fill` and `check` now reach a control by focus (Space for a checkbox). mutation: `check` back to `page.click`, AC2 leg at 1280x945 (the checkbox fully under the bar) → red at `check`'s `waitForFunction`, the CI timeout; the focus form green at 1280x945 and 1280x900. mutation: the store's `metadataWire` drops `client_credentials` from the grants, rebuilt and redeployed → AC2 red on `GrantTypes` (AC2).
- Rework 1, `[CI]` item 2 (latent since Story 15.9, outside this story's code): `account-and-filter` read the command box right after the wait for the Sign out row, but "s" already draws that row, so a render landing part-way through the typing left the read on a stale "si" list (measured: value "sign out", active the Users row; correct when read again 1.5 s later). The test now waits two frames before reading. mutation: the typing split as CI is taken to have interleaved it (inference; CI reports only `active: false`) ("si", its render, then "gn out") → red with CI's `active: false` without the wait, green with it; `CommandBox.activeDescendant` never names the account row, rebuilt and redeployed → red (AC: Story 15.9's Sign out row). Each client mutation reverted (md5 identical), rebuilt (`main-LOJ2LBGM.js`) and redeployed; CI's first three files 20/20 and the six OAuth files in CI's order 28/28.
- Code review (rework 1): mutation: `activeDescendant` answers `null` for the account row, rebuilt and redeployed (`main-N44S3LX7.js`) → Sign out test red with `active: false`; reverted (md5 identical), rebuilt `main-LOJ2LBGM.js` and redeployed; `account-and-filter` 4/4, `oauth-server-editor` 5/5.
