...
Code review 2026-09-13 (`full-opus`, four layers: blind-hunter, edge-case-hunter, verification-gap, acceptance-auditor). 46 raw rows → 15 root-cause entries after grouping: high 0, medium 7, low 8. 0 decision-needed. Rule 6: all nine named ADs verified against their Rules; AD-44, AD-5, AD-8, AD-19, AD-21, AD-43, AD-47 and AD-11 are satisfied on named evidence. Rule 1: the escape clause is correctly used. Rule 3: the CLI half has real-runtime evidence (a spawned-process test asserting exit code, stdout and empty stderr); the card has no render site until Story 9.9 and the browser gap is DW-159, not re-filed. Rule 5: passes — SM-C1's count line is the sanctioned path.

- [x] [Review][Patch] **The seventh vacuous pass: both build-side population sources key on the file path while the registry keys on the class name.** `readSources()` reads the module constant `DESCRIPTOR_DIR` and `descriptorFileNames()` reads the injected one, with a byte-identical filter — so on the gated path the `scanned !== fileCount` equality cannot fire, and the module header's "the population is asserted twice … cannot pass by looking at less than the tree" was false there. `Registry.Descriptors` selects `Name %STARTSWITH 'OcuPilot.Screen.Descriptor.'`, and `check_package_placement` asserts only that a class sits under one of the seven fixed packages — so `src/OcuPilot/Screen/Ssl.cls` declaring `OcuPilot.Screen.Descriptor.Ssl` is enumerated by the instance and invisible to `readSources()`, `descriptorFileNames()` and `descriptorSubdirectories()` at once. **Fixed:** `misplacedDescriptorClasses()` scans `src/OcuPilot/` by class name — a second source that can genuinely disagree — and refuses; the header now states what each of the three assertions actually does. [ui/tools/classic-links.mjs, ui/tools/classic-links.test.mjs]
- [x] [Review][Patch] **The Integration AC's mirror coupling was enforced by a runtime throw a redundant cast would mask.** `...(HOME as ScreenDeclaration)` silenced the type checker that the `if (HOME === undefined) throw` had already satisfied, so deleting the guard restored the unfalsifiable state with no compile error. **Fixed:** cast removed; deleting the guard is now `TS2322`. [ui/src/app/shell/classic-link-card.spec.ts:41]
...
- [x] [Review][Defer] The card's action has no visible hover — `.ocu-button-secondary:hover` and `.ocu-classic-link-card` are the same token. — deferred: **DW-185** `routed owner=9-9`; unreachable until the card has a render site.
- [x] [Review][Defer] The two refusal engines are kept in step by hand only, already disagree on `{"exempt": 1}`, and only the JS copy is gated. — deferred: **DW-186** `escalated`; the duplication is spec-bound, a shared corpus is a design call.
- [x] [Review][Defer] AD-27's Rule ("every screen keeps FR-9's link") is unsatisfiable for 12 of 16 archetypes under AD-44's vocabulary. — deferred: **DW-187** `escalated`; the story is right against AD-44 and `prd.md:298`, and Rule 20 calls an AD-vs-AD conflict a re-architecture.
- [x] [Review][Defer] An empty descriptor directory is a clean exit 0. — deferred: **DW-188** `wontfix-theoretical`.
- [x] [Review][Defer] The card's title is a `<p>` in an unnamed `<section>`. — deferred: **DW-189** `wontfix-accepted` with a probe.
- [x] [Review][Defer] The card's `present` guard does not restate the same-origin constraint. — deferred: **DW-190** `wontfix-theoretical`.

**Rejected.** `low`/`false`, one line each.

- `.ocu-command-bar`'s `min-width: 0` called a no-op "shipped with a comment saying it does nothing" — `false`: the comment states it is defensive and names `.ocu-shell-content` as the real bound, which is what the test asserts.
- The refusal assertions match the bare archetype key as a substring — `low`: no declared key is a substring of the fixed message text, and asserting the surrounding quotes would couple the oracle to message punctuation.
- The spec's corrupted duplicate `## Design Notes` heading at `:229`, orphaning nine triage rows — real, but the fix is a spec edit; raised to the lead instead (see the review's closing summary).
- `## Auto Run Result`'s counts (480/480, 19/19, "20 tests") are stale against the tree (478 → 480, 19/19, 23) — same: a spec edit, raised to the lead.
- `Test/Descriptor.cls` at 529 lines / 19 methods exceeds the ~500-line guidance — `low`: marginal, and splitting a live suite risks the shared-fixture race the project has scar tissue about.
- The I/O matrix's "`Registry.Validate()` returns the same sentence" is false (quoting, and materially for the unknown-archetype case) — folded into DW-186 as evidence; the fix is a spec edit or cross-language message churn.
- The `Never` clause "no second archetype list, no second exemption predicate" contradicts the delivered code — `low`, spec-bound: the Approach asks for both sides and the triage log closed it `by-design`.
- `buildMirror`'s `archetypeKeys.length === 0 ? 'never'` fallback is unreachable — `low`: a dead safety net whose only fix is deletion.
- A directory symlink inside `DESCRIPTOR_DIR` bypasses the sub-package refusal — `low`: not everyday, and the precise guard adds branching.
- Three of six assertions in QA's empty-tree test are forced by its input — `low`: the load-bearing ones (`ok`, and three report matches) can fail.
- The real-tree cross-check's `scanned === fileCount` assertion is implied by `ok === true` — `low`: redundant, not wrong.
...
- **(QA, mutation corrected at review)** DW-183's shape is pinned at the seam this suite can reach, not at `readSources()` → `ui/tools/screen-mirror.test.mjs` "DW-183: extractXData returns a body a bare JSON.parse rejects without naming a file". mutation: replace `extractXData`'s body capture with one that stops at the first `}` → `body` comes back `null` and the first assertion goes red. The line previously recorded here ("changed the synthetic source's body to valid JSON") mutated the test's own fixture rather than the code under test, so it demonstrated only that the literal is load-bearing. The test never calls `readSources()` — which takes no directory argument, so no test here can point it at a malformed descriptor — and therefore will NOT redden when DW-183 is closed in Story 2.3; the entry's `occurrence` trailer records that, and the test's own header now says it.
- **(QA)** an empty descriptor directory is a clean run of zero, not a refusal → `ui/tools/classic-links.test.mjs` "an empty descriptor directory (besides the base) is a clean run of zero, not a refusal". mutation: returned early on `scanned === 0` before the report lines were pushed → that test went red on "the zero count is printed, not omitted". (That the zero is not itself refused is DW-188, closed `wontfix-theoretical`.)
- **(QA, mutation corrected at review)** the closed vocabulary's agreement is exhaustive, not sampled, against an oracle independent of `Archetype.MayLinkOut` → `OcuPilot.Test.Descriptor:TestEveryArchetypeKeyAgreesBetweenRegistryAndTheVocabularysOwnClassification` (live). mutation: changed `panel`'s `linkOut` from `none` to `detail` in `Archetype.cls`, recompiled, ran the class → **exactly one** test went red, this one, naming `panel`; all eighteen others stayed green, including the three-key sampled `TestAClassicLinkDeclarationOutsideThePermittedShapesIsRefused` and `TestTheArchetypeVocabularyIsClosedAndClassified`. `panel` is one of the eight keys no other live assertion names, which is what makes it discriminating; the `drill-down` mutation recorded here before also reddens `TestTheArchetypeVocabularyIsClosedAndClassified` and two JS tests, so it showed the test can fail but not the asymmetry claimed for it. Reverted, recompiled, re-ran: 19/19, confirmed by the numeric-run-index `%UnitTest_Result` probe; `git diff --stat HEAD -- src/OcuPilot/Screen/Archetype.cls` empty afterwards.

- **(review)** the check cannot pass by looking at a tree no reader saw → `ui/tools/classic-links.test.mjs` "a descriptor class declared outside the descriptor directory is refused -- the case the equality guard cannot see". mutation: guarded the `misplaced.length > 0` refusal with `false &&` → exactly that test went red. Second mutation, for the report line the same patch added: deleted the `scanned 0 descriptor(s)` push from `refusedBeforeClassifying()` → the same test went red on `/^classic-links: scanned 0 descriptor\(s\)$/m`.
- **(review)** the Integration AC's mirror coupling is enforced by the compiler, not by a runtime throw a cast can mask → `ui/src/app/shell/classic-link-card.spec.ts`. mutation: deleted the `if (HOME === undefined) throw` guard → `npx tsc --noEmit -p tsconfig.spec.json` failed with `TS2322 ... Type 'undefined' is not assignable to type 'string'` at the `EXEMPT` literal. Before the review removed the redundant `as ScreenDeclaration`, the same deletion compiled and every Integration-AC assertion went on passing over a hand-built literal wearing a spread.
...
- `ui/tools/classic-links.mjs` + `classic-links.test.mjs` -- new; the check and its 23 tests.
...
480/480; `npm run test:components` 184/184 over 19 files; `npm run build` succeeds with the check in
...
and `bash scripts/lint-docs.sh` clean. Live on `ocupilot-iris`: `OcuPilot.Test.Descriptor` 19/19 and
