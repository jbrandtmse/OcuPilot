...
baseline_commit: '127c79e92dc0165381f9296fe729c24c49aa1670'
...
- **`src/OcuPilot/Test/Wire.cls:387`** — `$Get(tVerdict("os-management")) = 1` for the `%Admin_Operate`-only `#ADMINUSER`. Adding the pairs **flips this to a denial**; rewrite it as a `failedPair` assertion. Easy to miss and it is a real behavior change for that principal. New denial leg on the template at `:521-550`.
...
- `ui/src/app/shell/command-bar.ts` (+ its spec and `ui/src/styles/_components.scss` if the menu needs a class) — add the sort control: shown only when the screen's declaration carries `read.sort.fields`, labeled `sortMenuLabel`, offering each declared sort field under its **column's own `labelKey`** plus the two directions, and calling `store.setSort` / `store.setDirection`. Ordinary Tab stop; no new keyboard binding; the data-table is untouched.
...
**This pass (rework iteration 1, after the amendment).** No production behavior changed for the
