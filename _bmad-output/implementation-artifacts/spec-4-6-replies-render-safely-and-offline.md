...
### 2026-09-18 -- Code review (job 4-6-review-1)

- verdicts: 9 patched (8 medium, 1 low), 5 ledgered, 0 high; four layers ran, none failed
- patched:
  - `[medium]` `parseReply` threw `RangeError` on 1,500 nested `>` (3,001 chars) or 3,000 nested `*` -- both inside AD-24's text cap -- so the `effect()` left the reply blank; it now answers the literal source.
  - `[medium]` The literal-source fall-through (GFM table, raw-HTML block, thematic break) collapsed to one run-on line once `white-space: pre-wrap` was dropped; those paragraphs now carry `ocu-reply-source`.
  - `[medium]` The host caption was a child of the `a`, making its accessible name `docsdocs.example.com`; it is now the sibling DESIGN.md and the I/O matrix specify.
  - `[medium]` A GFM task list's `checkbox` token fell through to the block rule, putting `[ ]` in a paragraph of its own above each item.
  - `[medium]` A reply link had no `:focus-visible` rule (EXPERIENCE.md Component Patterns).
  - `[medium]` The AC2 notices assertion could not fail for `marked`: dompurify's Apache-2.0 text carries "conspicuously marked". It now matches the extractor's anchored `Package:` line.
  - `[medium]` `panel.spec.ts`'s mutual-exclusion assertion could not fail (its failed fixture carried no reply) and the invariant was unenforced on the restore path; `turns` now decides it.
  - `[medium]` None of the 138 new lines of reply CSS was verified; `reply.browser-spec.mjs` (a2) now reads the computed host display, keyword weight, comment slant, code surface, focus rule and source whitespace.
  - `[low]` The class allow-list test read back only classes a parse emits, so deleting `filterClasses` stayed green; `isAllowedClass` is exported and pinned directly. `build-output.test.mjs` now reddens on an emitted initial chunk it does not measure.
- ledgered: DW-1085 (dropped `start`/`title`, wontfix-accepted), DW-1086 (per-spec fixture-helper copies, routed 4.7), DW-1087 (`no-off-origin-url` defeated by concatenation, escalated), DW-1088 (`hast` via lowlight only, wontfix-theoretical), DW-1089 (entities render literally, by-design)
...
- AC1 → `reply.test.mjs` "a fenced sql block highlights". Mutation: make `parseReply` ignore the `code` token's `lang` → the `hljs-keyword` assertion reddens. AC1's *visible* half (Release 1 highlighting is weight and slant, D3) → `reply.browser-spec.mjs` (a2)'s computed-style reads. Mutation: delete `_components.scss`'s `.hljs-keyword { font-weight: 600 }` rule → "expected an hljs-keyword heavier than plain code, got 400 against 400", while every class-presence assertion in the suite stays green.
...
- AC2 → `build-output.test.mjs`'s notices test, which matches the extractor's own anchored `Package: <name>` heading line. A bare `includes('marked')` could not fail: the Apache-2.0 text `dompurify` ships carries the phrase "conspicuously marked" twice, so `3rdpartylicenses.txt` names `marked` whether or not it is bundled. Mutation: drop `marked` from `dependencies` → the heading line disappears and this reddens. `reply.browser-spec.mjs` (c) is a forward guard, not a pinning test: its only assertion is the list-item count, so it reddens if a dependency ever fetches off-origin at render time and there is no request-log assertion to mutate.
...
- AC4 → `reply.spec.ts` "an external link carries its href, rel, no target, and a host caption after it" and `reply.test.mjs` "an external link carries its href, and its bare host follows it as a sibling caption". The caption is the anchor's SIBLING, per DESIGN.md `message-agent` ("the full host visible … after the link text") and this matrix's External link row: nested, it joins the link's accessible name and sits inside its activation area. Mutation: return `[a(label, caption)]` instead of `[a(label), caption]` from `linkNodes` → the anchor's own text is no longer `docs` and both redden. The `target` half has no falsifiable single-point mutation — the builder never writes `target` AND `ALLOWED_ATTR` omits it, so either guard alone forces `null`; it is defense in depth, recorded as such.
...
- AC7 → `panel.spec.ts` "a stopped turn renders no error banner". Mutation: make `turnErrorBanner` answer the banner for `state === 'stopped'` → red (this is already `turn.test.mjs`'s recorded mutation; the panel-level pin is the surface half). AC7's mutual-exclusion half → `panel.spec.ts` "a reply and an error banner never both render for one turn", whose failed turn CARRIES a reply — the state the invariant is about, and one the wire produces: `Kernel/State/Turn.cls` nulls a non-completed turn's reply on the live view, but `Kernel/State/Entry.cls` emits whatever the row stored and `Kernel/Agent/Job.cls` records the loop's reply alongside a `failed` state, so a reload hands the panel both. Mutation: drop the `errorBanner === null` guard on `reply` in `panel.ts`'s `turns` getter → red ("expected <app-reply> to be null").
