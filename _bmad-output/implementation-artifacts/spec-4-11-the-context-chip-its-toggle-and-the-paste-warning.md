...
- Cap follows `agent-switch`: remove the bus subscription from `agent-context.ts` -> the cap leg reddens. **Applied 2026-09-17 (review): red** -- `context-chip.browser-spec.mjs`'s cap leg timed out at `:477` against a rebuilt, redeployed mutant bundle; reverted and green. The `panel.spec.ts` twin has no bus and is reddened instead by removing `agentContext.subscribe` from `context-chip.ts`.
- Acknowledgment survives a failed send: restore `syncSecretRecord`'s shape-only check -> `panel.spec.ts`'s conversation-500 case reddens. The live 409 leg cannot stand for this one: `TurnStore.send()` creates the conversation before it POSTs, so `conversationId()` is never null there.
...
**Follow-up review recommendation: true.** Two or more medium-severity entries were patched in this pass (the `onScopeChange` subscription, the live-cap component test, and the `syncSecretRecord` reset-detection fix), which this template's rule makes `true` regardless of how thoroughly the patches were re-verified here. Both named risks were then closed: the QA commit added the two browser legs, and code review demonstrated the cap leg red under its mutation. The 409 leg proves the observable behavior live but not the `syncSecretRecord` guard, whose pin stays the component test -- recorded in `## Verification`.
...
- Rebuilt (`npx ng build`), `docker cp`'d into `ocupilot-ci`, confirmed the served bundle hash via `curl`, then ran against the live throwaway: `context-chip.browser-spec.mjs` 8/8 (6 at implement, 8 once the QA commit's two legs landed), `panel.browser-spec.mjs` 10/10, `turn.browser-spec.mjs` 8/8. The full ordered suite is 107/107 (code review, 2026-09-17).
...
**Residual risks:** the pre-existing bundle-size budget overage (143.12 kB after this review's patches, 142.89 kB before), unrelated to this story in nature. Live coverage for AD-42's `agent-definition` re-read is ledgered as DW-1076.
...
### 2026-09-17 — Code review (job 4-11-review-1)

Four layers raised 59 findings, grouped to 31 entries by root cause: high 2, medium 3, low 5,
false 4, closed at emission 16, decision-pending 1. All 10 real defects were patched in the review
pass and each is pinned by a test demonstrated red under a named mutation; nothing is left as an
action item. Gates after the patches: `npm test` 890 + 443 green, `npm run build` clean, the full
ordered browser suite 107/107 against the rebuilt and redeployed `ocupilot-ci` bundle,
`check-objectscript` 0, smoke 18/0/2 pending.

- [x] [Review][Patch] **HIGH** The new browser spec leaves an enabled, default-marked turnprobe definition on the instance, which reddens `switches.browser-spec.mjs` AC2 in the CI order [ui/browser/context-chip.browser-spec.mjs:57] — `priorDefault` was read with a bare `Write`, so it held the whole IRIS session transcript (`"Node: …, Instance: IRIS\n\n%SYS>\n\nHSCUSTOM>\n925\nHSCUSTOM>"`); embedded in the `after()` hook's ObjectScript string literal that breaks the script, so `RemoveDefinition` never ran and its status was never read. `context-chip` is 3rd in the glob order and `switches` 16th, and AC2 asserts the panel's read-only line "starts at the off key" on the stated assumption that nothing is configured. Observed: after the spec, `GET /agent/definitions` returned the probe definition (`model: chipbrowser6`) and `switches` AC2 failed `'Read-only: on — by the definition' != 'Read-only: off'`. Fixed: `markedDefault()` reads through `markerValue()`, and `removeDefinition()` asserts `RemoveDefinition`'s status. After: the spec leaves `{"definitions":[]}`, `switches` is 4/4, and the full ordered suite is 107/107. Neither 4.11 commit had reached CI, so nothing had caught it.
- [x] [Review][Patch] **HIGH** AD-42: the chip's provider, endpoint host and egress pill went stale on a moved default marker [ui/src/app/core/agent-context.ts:278] — `provider`, `endpointHost` and `leavesInstance` are the *default definition's*, and the default marker moves on `agent-definition` (`definition-actions.ts:157`) or a form Save (`definition-form.store.ts:689`), never on `agent-switch`. `onChange` listened only for `agent-switch`, so marking another definition default left the chip naming the previous host — or showing no "leaves the instance" pill for an endpoint that now has one — until a reload, which AD-42's "it cannot disagree with where the request actually goes" forbids. `AgentStatus.onChange:392` already listens to both. Fixed: both types re-read; `agent-context.test.mjs`'s AD-14 test rewritten and demonstrated red under dropping the new leg. Live coverage ledgered as DW-1076.
- [x] [Review][Patch] **MED** `setShare()` broke the invariant `read()`'s superseded branch depends on, so an overtaken `load()` never settled [ui/src/app/core/agent-context.ts:218] — it bumped `request` without moving `newest`, so a read it superseded reached `await this.newest` and awaited its own promise. Reachable whenever a toggle click overlaps the startup read or an `agent-switch` re-read; 14 test sites already `await agentContext.load()`. Fixed: `setShare` is a thin wrapper that assigns `newest`; new test times out (8 s) under the mutation.
- [x] [Review][Patch] **MED** The 409 browser leg claimed to prove the `syncSecretRecord` guard and cannot [ui/browser/context-chip.browser-spec.mjs:529] — `TurnStore.send()` creates the conversation before it POSTs (`turn.ts:402-411`), so `conversationId()` is non-null at the 409 and the guard's `current === null` leg is false under both the old shape-only check and the new one. Fixed: the comment now states what the leg does prove live and names `panel.spec.ts`'s conversation-500 case as the guard's pin; `## Verification` carries the mutation.
- [x] [Review][Patch] **MED** `Panel`'s `onScopeChange` subscription — the previous pass's own medium fix — was pinned by nothing [ui/src/app/shell/panel.spec.ts] — `StubScope.notify()` existed and was never called. Fixed: a test that moves the namespace after mount; red under dropping the subscription from either `Panel` or `ContextChip`.
- [x] [Review][Patch] **LOW** A URL resolving to no built descriptor rendered the chip as ", HSCUSTOM" [ui/src/app/shell/panel.ts:521] — reachable through `app.routes.ts:78`'s `**` route, which keeps the panel mounted, while `assembleScreenContext` posts nothing for the same URL. Fixed: the gate now also requires a resolved descriptor.
- [x] [Review][Patch] **LOW** The cap test recorded a Rule 19 mutation it cannot perform [ui/src/app/shell/panel.spec.ts:1353] — it constructs `AgentContext` with no bus, so removing the `agent-switch` re-read cannot redden it. Fixed: the comment names the mutation that does (`agentContext.subscribe` in `context-chip.ts`, demonstrated red) and points at the store-level and live pins for the bus wiring.
- [x] [Review][Patch] **LOW** DESIGN.md's `context-chip` "When off" state was not implemented [ui/src/styles/_components.scss:3353] — `--ocu-restrained` applied only under the kill-switch class, so the sharing-off sentence kept `--ocu-on-surface`. Fixed: `.ocu-context-chip-off` colors the sentence and leaves the border to the kill switch.
- [x] [Review][Patch] **LOW** Both new browser legs mutated shared state before entering their `try` [ui/browser/context-chip.browser-spec.mjs:434] — `putContextRowCap(1)` is instance-wide and the holder tab occupies AD-41's one turn slot for 15 s, so a throw before the `try` left the cap at 1, or the slot held, for every later spec. Fixed: both moved inside the `try`, with optional-chained teardown.
- [x] [Review][Patch] **LOW** An acknowledged paste warning carried into a fresh conversation [ui/src/app/shell/panel.ts:646] — `newConversation()` settles a *new* non-null id, which `syncSecretRecord()` cannot read as a reset, and the review-pass item that named "the explicit sign-out / new-conversation paths" was only half applied. Fixed: `onNewConversation()` clears the record.

**Rejected.**

- `false` — the chip keeps a subscription to a discarded `ScreenStore` after sign-out: `ScreenStores.reset()` runs only from `refresh.reset()` (`app.ts:421`), inside the not-signed-in branch, and `app-panel` sits inside `@if (signedIn)` (`app.ts:153`), so the chip is destroyed on that path.
- `false` — `syncSecretRecord()`'s clear branch is unreachable and its `lastConversationId` bookkeeping dead: `endSession()` notifies synchronously, before Angular re-evaluates `@if (signedIn)`, so the branch does fire once on sign-out.
- `false` — `contextChipScreenSegment` is a Fixed strings row nothing renders: EXPERIENCE.md:261 calls it the screen-segment *pattern* and `panel.spec.ts` pins the composed sentence byte-for-byte against it. Every other chip and warning literal maps to a row, and no row this story owns lacks a renderer.
- `false` — `narrowRow`'s doc contradicts its code on non-object rows: the doc says the row's *members* are dropped, which is what returning `{}` does.
- `wontfix-theoretical` — the browser legs' `/(\d+) rows/` misparses a grouped count: both reads are bounded by a cap of 1 then 200.
- `wontfix-theoretical` — a malformed `contextRowCap` posts zero rows: the value comes only from `GET /agent/context`, which the server clamps to 1–1,000.
- `by-design` — `looksLikeSecret` misses JWTs and slash-bearing secrets, fires on a UUID, and anchors its prefix leg at position 0: the algorithm is specified byte-for-byte in the frozen `<intent-contract>`.
- `by-design` — `sort`/`direction` are posted raw rather than resolved through the descriptor's defaults: pinned as intended by `screen-context.test.mjs:130`.
- `by-design` — "1 rows" has no singular form: settled in `## Design Notes`.
- `by-design` — the sharing switch is named twice (a visually-hidden span inside the `<label>` and an `aria-label` on the input): the same `.ocu-visually-hidden` sibling convention the previous pass closed as `false`.
- Also rejected, each a re-statement of a settled owner decision or of a `## Design Notes` boundary: the inline warning's live region not pre-existing, the warning reusing the informational glyph, the chip having no overflow rule, the switch's target size, `shareDefault()`/`userChoice()` having no renderer, `currentEntityId()` being a copy of `locator-bar.ts`'s, the browser spec re-implementing `parseMarkers()`, and `stubAgentContext` not settling `userChoice`.

**Decision-pending (DW-1077, for the epic decision sheet).** On Home the chip names a screen and namespace that no turn carries: Home's declared route is `''`, `Api.Turn.ContextViolation` refuses an empty route, and EXPERIENCE.md:712 mandates the chip text. Under-claims rather than over-claims, and every exit costs a server change or a UX contract.
