...
- findings:
  ...
  - `[medium]` `[patch]` `Event.Description()` fell through to the grant marker's description for any name it did not recognize, so a fourth roster entry would register under a sentence about something else — it now names itself instead.
