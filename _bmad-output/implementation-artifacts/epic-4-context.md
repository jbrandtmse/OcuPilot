...
and progress store), 4.2 (tool advertising, dispatch, the gate point, the three shell reads), 4.3 (the
docked panel on every route) and 4.4 (screen context accepted, bounded and injected server-side).
...
Processing order after 4.4: 4.5, 4.11, 4.6, 4.7, 4.8, 4.9, 4.10.
...
- **The turn never holds a request open.** `POST` returns a turn id at once; a background job does the
  work and writes only OcuPilot's own state.
...
- **Screen context** is built by the client fresh at Send (4.11) and already accepted server-side:
  route, namespace, selected entity, and the screen's filtered and sorted view with its sort and filter.
  The kernel bounds context and every read tool result (row cap `contextRowCap`, 1-1,000, default 200;
...
- **Status read** answers `provider`, `endpointHost` and `leavesInstance` from the provider port's own
  resolution (local only when marked local or every address is loopback, link-local, RFC 1918 or
...
- **Governing ADs.** 4.5: AD-11, 19, 33, 41.
...
- **Dispatch.** Every registered tool is advertised, in name order; `Loop.AnswerTools` answers one
  ...
  `Registry.InvokeTool`. Results return as bounded `tool_result` content or `{code, detail?}`, within a
  65,536-character tool-result budget per reply.
...
- **Prompt and untrusted text.** System prompt is a build-time constant (`Kernel/Agent/Prompt.Builtin()`);
  ...
  `screen_context` tool call and result before the user's message, canonical message shape, never
...
- **Errors and routes.** Envelope `{error, reason, code, detail}`: screens render `reason`, tool results
  `code`. `POST /api/ocupilot/turn`, `GET …/turn/{id}/progress`, `POST …/turn/abandon`,
  `GET/PUT …/agent/context` exist; every handler has an HTTP integration test.
...
- **From 4.3.** `ui/src/app/shell/panel.ts` renders empty slots; `core/panel-layout.ts` holds
  ...
  the sole layout computation. 4.5 wires Send and Stop, the lock
  ...
  `resolveLayout`. Epic 5 fills "not being marked". Decision-pending: DW-458 (affects 4.10) and DW-460.
- **From 4.4.** `POST /api/ocupilot/turn` takes an optional `context` object: `route`, `namespace`
  (must match the request's `?ns=` scope, else 422 `TURN.CONTEXT.INVALID`), `entity`, `view` with `rows`,
  `sort`, `direction`, `filter`, `rowsAvailable`. `Screen/Context` projects it by descriptor
  (`context.fields`, `context.secretFields`, optional `context.maxLength`). `Kernel/Agent/Bound` applies
  row cap, total and per-field cuts to context and to every rows-shaped read tool result (class tools
  included), reporting `rowsSent`, `rowsAvailable`, `truncated`, `truncatedFields`.
  `GET/PUT /api/ocupilot/agent/context` answers and stores `share`, `shareDefault`, `userChoice`,
  `contextRowCap`, `provider`, `endpointHost`, `leavesInstance` (per-user store `Kernel/State/Sharing`);
  `contextRowCap` lives on Switches (`Kernel/State/Switch`, the Switches page) and also bounds tool rows.
  4.11 (after 4.5's Send) assembles that POST shape at Send and drives chip and toggle from
  `/agent/context`; 4.5's read cards show `rowsSent`/`rowsAvailable`/`truncated`.
...
- **From 4.2.** 4.5 owns DW-451 (a tool step records neither rows sent nor the failed pair
  `detail.failedPair`) and DW-1048 (flaky sign-out browser leg: wait for the composer). 4.7's navigation tools register through the
...
- **Ledger entries** routed onto a story bind it: address each or decline with a reason. DW-456 is
