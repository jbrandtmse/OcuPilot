...
- **Read and table grammar.** `read`: `source` (`port: admin`, `endpoint`, `type: LIST`, optional `rowGet`), `fields`, `filter`, `sort` (`fields`, `default`, `direction`), `paging: cap`, `table` (`columns` of `{field, labelKey, kind}`, kinds `name`/`identifier`/`text`/`number`/`status`, exactly one `name`; `emptyNextKey`, or `emptyAgentKey` only when a primary or row action is declared). No secret field is a column, filter or sort. No cursor paging, so 2.11 would extend the grammar (inference). `Registry.ReadProblem` and `screen-mirror.mjs` `readProblem` hold identical refusal sentences; change both, and pin new ones in a shared corpus class the way `Test/RowGetCorpus.cls` and `Test/AdminPairCorpus.cls` do.
- **A list screen is one descriptor class** directly in `Screen/Descriptor/` (the mirror reads it flat), then `node ui/tools/screen-mirror.mjs`; `ListPage` renders it, with no per-screen route, handler, tool class, page, store or `areas/` file. `WebAppList`, `UserList` and `SslConfigList` are the templates: instance scope, `single` id, tool identifiers `webapp.list` / `permissions.users` / `security.ssl`, and `SslConfigList` is the no-`rowGet` shape. The gate does not union the area's pairs; each descriptor declares its own. Never set `navigates` on a list's area.
- **The admin-port read pair set is grammar.** `%Admin_Secure:USE` + `%DB_IRISSYS:READ` together, each necessary and both sufficient (throwaway, real principal). Settled at the mechanism, not by extrapolation: `AdminPort.RunSequence` switches to `%SYS` for every request type, and `%SYS`'s `%DB_IRISSYS` has no public permission — so `ReadProblem` and `readProblem` each refuse, as their **last** check, an admin-port read whose `privileges` omit `%DB_IRISSYS:READ`, one identical sentence pinned by `Test/AdminPairCorpus.cls` in both engines. Every admin-port descriptor and every read fixture declares the pair. Never `%DB_IRISSECURITY`: IRIS refuses to grant it to a role (probed).
- **An area covers its screens.** `Registry.AreaCoverageProblem(pAreaKey, pPairs)`, called from `Validate` per descriptor right after `MalformedPair`, refuses a screen declaring a pair its area does not, naming class, area and pair; `Test/AreaPair/Bad.cls` + `AreaPairRegistry` is the falsifying fixture. Containment only — a surplus area pair stands (a false denial over a false promise). `web-applications`, `permissions` and `security` declare `%DB_IRISSYS:READ` beside `%Admin_Secure:USE`. Deliberately no `screen-mirror.mjs` twin: `Validate` is the roster check CI runs.
- **Key material stays out by shape.** No `rowGet` means no GET is ever issued, and `Read.Project` copies only `read.fields`; `context.secretFields` is for what a read does carry. `screen-mirror.test.mjs` refuses, over every production descriptor, a `read.fields`, `filter`, `sort.fields`, `table.columns[].field` or `context.fields` name matching `field-lists.mjs`'s `CREDENTIAL_RE` — a suffix-anchored vocabulary that catches `PrivateKeyPassword` but not `PrivateKeyFile`, `PrivateKeyType`, `CertificateFile`, `CAFile` or `CAPath`, so any detail read over those names pins them by hand (DW-268, widened in 9.5).
...
- **Tripwires per list.** `Test/ReadTool` pins the production tool list by content (today exactly `permissions.users.read`, `security.ssl.read`, `webapp.list.read`); `Test/Descriptor.cls` pins every area's privilege pair set, `navigates` and `pinBottom` by content; `navigation.test.mjs`, `navigation-wire.test.mjs` and `rail-wire.spec.ts` pin built routes and `LIVE_PAYLOAD`; `Install.Smoke` runs one live screen read per built list (`webapplications`, `users`, `ssl`), each check pinned to the screen it actually read (its report line names the check, not the tool, so a transposed lookup would otherwise stay green), and its `arealists` pending note names the areas still to come (Logs, OS management, Tasks).
...
- **Order.** 2.8–2.9 copy the three list templates' shape. 2.8 owns DW-267 (the list browser specs' filter step is triplicated and vacuous in the users and web-applications copies — a `filterTo` leg is satisfied by the rows the previous leg left; share one helper, and make each leg run from the whole list and fail when the filter matches nothing). 2.8 is also the first list to declare auto-refresh, and the first with a banner and a privilege-gated action above the table, so `refreshes`/`refreshRates` and the command bar get their first non-empty values on a list. 2.9's persisted sort needs a sort control or rests on the declared default (inference). 2.10 runs the async LIST through `Execute` plus server criteria and carries the 1,000-row timing. 2.12 uses 2.11's `LogSourcePort`.
