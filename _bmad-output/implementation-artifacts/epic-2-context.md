...
- **Denial and demo checks run on the throwaway.** Under `%All` every check passes, so a denial needs a real principal: `Test/Wire.cls`'s `%Admin_Operate` user, or a browser spec on `scripts/ci-throwaway.sh` (`ocupilot-ci`, `OCUPILOT_DEMO=1`) that refuses the live container and deletes its principal. No security-object mutation on live `ocupilot`.
...
- **Side bar.** Choosing a screen in the command box moves the side bar only when it is open (a collapsed bar stays collapsed; navigating areas such as Home are skipped); the locator's area segment opens it on that area.
...
- **Order.** 2.6–2.9 copy `WebAppList`'s shape. 2.6 owns DW-186: one shared JSON link-out corpus both engines read, a test per engine, the ObjectScript copy kept. 2.9's persisted sort needs a sort control or rests on the declared default (inference). 2.10 runs the async LIST through `Execute` plus server criteria and carries the 1,000-row timing. 2.12 uses 2.11's `LogSourcePort`.
