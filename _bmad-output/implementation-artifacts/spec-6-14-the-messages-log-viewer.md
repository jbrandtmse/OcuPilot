...
- [x] [Review][Patch] Scoped re-review, baseline `22d49cb`: the rework's stated basis was falsified. The comment and the Verification line said a refreshing screen re-renders the banner every tick and that the press landed on a detached node; auto-refresh is off until the chip sets a rate, a faulted read suspends the timer, and a probe on Processes and on Locks with the read refused replaced the banner element zero times in 25 s after one refused read. So the fault-source swap removes no race -- only the handle press changed behavior, turning a silent absorb into a named failure. Both claims replaced with what is known, and the press is now preceded by a named wait for the control un-gated, which is what tells a gated control (a handler that returns silently) from a lost press on a third CI failure [ui/browser/messages-log.browser-spec.mjs:329, :358] -- residual deferred: DW-1156, `escalated owner=burndown`, arrived after the burn-down gate
- [x] [Review][Patch] The first named wait still said the **processes** read was refused, in a test that no longer reads processes -- the DW-1119 class of unquotable failure line the rework exists to remove [ui/browser/messages-log.browser-spec.mjs:351]
- [x] [Review][Patch] The Verification section kept the superseded first-rework mutation bullet verbatim beside its replacement (the same 450-character dump twice) and added a paragraph narrating what the first rework achieved, in a spec whose frontmatter carries `warnings: ['oversized']` -- against CLAUDE.md Prose discipline, "Delete the wrong sentence; do not append a paragraph explaining that it was wrong" [spec-6-14-the-messages-log-viewer.md `## Verification`]
- [x] [Review][Accept] `handle.click()` is a coordinate dispatch with no ownership check, where this suite's own `clickRowCentre` marks the target, rejects a zero-area box and verifies `elementFromPoint` before clicking; a press swallowed by a covering element would read as the navigation wait's timeout. Accepted: no overlay exists on this screen, the press was observed to land, and the named navigation wait reports it [ui/browser/messages-log.browser-spec.mjs:370; ui/browser/list-spec.mjs:167]
- [x] [Review][Patch] The Verification line quoted `"rows":571` from the mutation dump as if the figure were part of the message; the same mutation re-run on this pass answered 468, so the count is the live tail's and a reader cannot use it to tell one run from another (CLAUDE.md, "Never mark a fact verified that you did not check") [spec-6-14-the-messages-log-viewer.md `## Verification`]
- [x] [Review][Accept] The handles `page.$$` returns are never disposed and the loop `break`s with the rest outstanding; `context.close()` in the `finally` releases them, and the banner renders two buttons [ui/browser/messages-log.browser-spec.mjs:362]
...
- mutation (AC8, re-demonstrated at this rework): `fault-banner.ts`'s `ShellState.showArea` call removed, bundle rebuilt and redeployed -> the wait reddens with `timed out waiting until the side bar followed to the Logs area with messages.log current; the page showed {"path":"/ocupilot/logs/messages","eyebrow":"OS management","current":null,"banner":false,"rows":<n>}` (observed; `rows` is the live tail's own rendered count -- 571 at the first rework and on this pass's first run, 468 on its second, so it is not a fixed figure and a quote of it is not a second run's fingerprint). Reverted, rebuilt, redeployed, spec file 4/4 green, and the 26-file browser suite 135/135 on `ocupilot-b-ci`. AC8's shape at this pass: the fault is raised by failing the **Locks** read (`LockList` declares `refreshes: false`), and the control is waited for un-gated and pressed through a Puppeteer element handle, so a gated control, a detached node and a lost press each fail by name.
- open, and not what this pass fixed: what made CI run 35378660401 absorb the press. A refresh tick was not it -- auto-refresh is off until the chip sets a rate (`preferences.ts` answers 0 unstored) and a faulted read suspends the timer, and a probe on both screens with the read refused replaced the banner element zero times in 25 s after one refused read (observed). DW-1155 carries the banner-identity defect and its note records this probe; DW-1156 carries the unexplained failure itself, `escalated owner=burndown`.
