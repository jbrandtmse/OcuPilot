...
**Round 1 (code review, 2026-09-13).** Four layers at `full-opus`, 40 rows → 28 root-cause entries:
high 0, med 4, low 24. Nine patched here, five ledgered, fourteen closed on their refutation.
Ten mutations applied, observed red and reverted; tree byte-identical after each.

**Patched (all applied in this pass):**

- [x] `[Review][Patch]` **A read still in flight when the namespace switches refilled the store it had just cleared, and its fault suspended the new namespace's timer** [`ui/src/app/core/refresh.ts`:364] — `noteScopeChanged()` cleared the rows and re-armed but left `issued` alone, so `tick()`'s two guards both passed and the left namespace's rows landed under a fresh `Last update` — the AD-44 switch undone by the read it superseded, and the one thing `tick()`'s own doc says the stamp must never do. A `404` about the namespace the shell had left also parked and suspended the one it was on. `this.issued += 1`; two new rows in `refresh.test.mjs`.
- [x] `[Review][Patch]` **A `proposal-open` whose `expiresAt` is already past silently disabled the pause** [`ui/src/app/core/change-bus.ts`:123] — `expiryFor()` clamped the ceiling only, so the subscriber's `sweepExpired()` dropped the id on arrival while `publish()` returned `true`: AD-43's pause never engaged and the diff under review would move. A publisher sending epoch *seconds* lands exactly there. The module doc and the test's own title already said "replaced by one that is"; the fourth assertion pinned the opposite. Floor added; three rows now pin it.
- [x] `[Review][Patch]` **AC 3's "nor any ancestor" was never walked over the shipped ancestor chain** [`ui/src/app/app.wire.spec.ts`:323] — both bar specs mount one component alone, so their walks stop at the TestBed root and never pass `.ocu-shell-content`, `.ocu-shell` or `app-root`. `aria-live` anywhere in the shell frame would announce every tick with the suite green. Walk added to the one fixture that mounts the chain, after a tick lands.
- [x] `[Review][Patch]` **`buildMirror` called an omitted refresh pair sound and then emitted a `SCREENS` missing two non-optional fields** [`ui/tools/screen-mirror.mjs`:314] — such a descriptor failed as an unreadable `tsc` error rather than at the named refusal. Defaulted after the spread, so a declaring descriptor emits byte-identically (`--check` still reads *up to date*).
- [x] `[Review][Patch]` **A `run` that threw synchronously wedged the single-flight slot permanently** [`ui/src/app/core/single-flight.ts`:73] — the slot is filled before `run` is called (the DW-157 property), so a throw before the promise skipped `.finally`, leaving `running()` true forever and every later `request()` joining a gate nothing settles. `try` around the call only, so `run` still executes synchronously.
- [x] `[Review][Patch]` **`screen-mirror.mjs` claimed the same five rules as `Registry.RefreshProblem`; it is stricter** [`ui/tools/screen-mirror.mjs`:227] — verified live on `ocupilot-iris`: `RefreshProblem(1, $ListBuild("10"))` returns `""`. `"10"` and `10` are one value in ObjectScript, so the instance cannot express the rule and the mirror can. The claim is corrected and the asymmetry named as deliberate; the refusing gate runs first, so the direction is safe.
- [x] `[Review][Patch]` **Rule 19: an inert assertion in the QA crossing** [`ui/src/app/app.wire.spec.ts`:286] — `expect(refresh.armedFor()).toBe('none')` sat in the block where `bind()` is never called, so it read the field initializer and no mutation to the arming logic could redden it. Removed; that test's real pins (chip and stamp null) carry its recorded mutation.
- [x] `[Review][Patch]` **The area timer scan matched only `setTimeout`/`setInterval`** [`ui/tools/refresh.test.mjs`:883] — rxjs `interval()`/`timer()` is the likelier way an Angular screen grows its own timer, and the scan read clean on it. Widened and demonstrated red against a probe module.
- [x] `[Review][Patch]` **`ScreenStoreOptions.preferences` was documented as "the four persisted slots"** [`ui/src/app/core/screen-store.ts`:52] — one is written, as the class header and the Spec Change Log both say. Corrected at the divergent sentence.
...
**Rejected (closed on their refutation):**
...
- (QA) The real routed Home screen draws neither consumer, since Home declares no refresh and nothing binds it → `ui/src/app/app.wire.spec.ts` "renders the real Home screen through the real router and mirror, with neither chip nor stamp". mutation: changed `RefreshService.refreshes()` to `return true` unconditionally → a live "Auto-refresh: off" chip rendered over Home and both assertions went red; reverted, byte-identical.
- (QA) One shared `RefreshService` drives the chip and the stamp together, composed around the real routed Home screen (the crossing the Auto Run Result's residual risk names) → `ui/src/app/app.wire.spec.ts` "Integration AC: one shared RefreshService drives the chip and the stamp together...". mutation: dropped `StatusBar`'s `stopRefresh = this.refresh.subscribe(...)` and its teardown call → the stamp assertion went red with no stamp in the DOM; reverted, byte-identical.
- (QA) Suspend-then-resume completes against the REAL `ConnectivityService` and the REAL `ApiService`, not the `{ retryWhenReachable(){ parks.push() } }` stub every other test in the suite uses → `ui/tools/refresh-connectivity.wire.test.mjs` "suspend-then-resume completes against the REAL ConnectivityService...". mutation: deleted `this.connectivity.retryWhenReachable(REFRESH_PARK_KEY, () => this.resume())` from `suspend()` → the real probe still armed and drained on schedule, but nothing was resumed and `armedFor()` stayed `'none'` forever; reverted, byte-identical.

- (cr) A read still out when the namespace switches cannot refill the store it cleared, and a fault from it cannot suspend the new namespace → `ui/tools/refresh.test.mjs`, two new rows. mutation: dropped `this.issued += 1` from `noteScopeChanged()` → both went red (the left namespace's rows landed in the cleared store under a fresh stamp; the superseded read's 404 parked and suspended). Reverted, byte-identical.
- (cr) An expiry already past is AD-6's ten minutes, not a pause that never engages → `ui/tools/change-bus.test.mjs` "an expiry that is not a moment within AD-6 is replaced by one that is". mutation: dropped `|| supplied <= nowMs` from `expiryFor()` → the past, epoch-seconds and this-instant rows went red.
- (cr) A `run` that throws synchronously settles the flight instead of wedging the slot → `ui/tools/single-flight.test.mjs`. mutation: restored the bare `void run(key)` → the row went red on a gate nothing settles.
- (cr) AC 3's "nor any ancestor" over the **shipped** shell chain → `ui/src/app/app.wire.spec.ts` Integration AC. mutation: `aria-live="polite"` on `app.ts`'s `.ocu-shell-content` → that row went red while `command-bar.spec.ts` and `status-bar.spec.ts` stayed green (17 of 18 files passed), which is exactly the gap it closes: their fixtures mount one component and the walk never reaches the shell frame.
- (cr) The area timer scan covers rxjs, not only `setTimeout`/`setInterval` → `ui/tools/refresh.test.mjs`. mutation: an `interval(1000)` module under `src/app/areas/home/` → the row went red and named it; removed.
