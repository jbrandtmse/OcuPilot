...
- `ui/tools/typography.test.mjs` -- the six roles and their values, no weight 700, no size below 11px, tabular figures, the scale and radii and metrics, the five `@font-face` declarations with local `src` and the exact fallback stacks, no external host in any `url()`, `@import` or `src` reference under `ui/src` (the vendored OFL license text carries a URL and is not such a reference), and the motion tokens with their reduced-motion override.
...
- [x] [Review][Defer] The vendored OFL license texts do not travel with the `woff2` faces into the redistributed bundle [ui/angular.json:1] — deferred: ledger DW-38, `decision-pending`; a legal/packaging call for the epic decision sheet, and the Task list says `angular.json` needs only the styles edit
