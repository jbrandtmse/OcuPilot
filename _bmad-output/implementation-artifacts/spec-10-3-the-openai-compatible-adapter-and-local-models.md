...
- **AC3, metadata arm (DW-1214)** -- a new `Egress` test over the metadata denylist. `mutation:` remove the metadata check so `IsPermitted` falls through to the marked-local branch -> red on `fd00:ec2::254` with the definition marked local. **The second half is obligatory:** the same test asserts a legitimate unique-local address (a private IPv6 LAN, not a published metadata address) is still **permitted** to a marked-local definition -- without it, a fix that refused all of `fc00::/7` would look identical to one that works.
