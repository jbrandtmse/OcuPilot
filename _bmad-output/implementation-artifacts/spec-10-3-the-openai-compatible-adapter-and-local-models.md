...
Code review of story 10.3 (2026-09-19), Tier 1 `full`, four layers. No `high` and no
`decision-needed`. Every `patch` entry below was applied in the review pass and is checked off;
`defer` entries are ledgered with an owner and do not block `done`.

- [x] [Review][Patch] The acknowledgment control is not gated on `markedLocal`, so it renders in a state the server refuses on `endpointUrl` and the control cannot clear [ui/src/app/areas/agent/definition-form.page.ts:843]
- [x] [Review][Patch] The cascade's `httpAcknowledged` clear-down had no assertion that can fail: the only one compared `false` against a field never set to `true` on that path (Rule 19) [ui/src/app/areas/agent/definition-form.page.spec.ts:1237]
- [x] [Review][Patch] The restored credential rung had no assertion that can fail: the only one starts and ends at `creds`, which is also what the behavior it replaced produced (Rule 19) [ui/src/app/areas/agent/definition-form.page.ts:920]
- [x] [Review][Patch] `Test/EgressLocal` deletes the instance's outbound-settings row in both hooks with no snapshot or restore -- the defect the same pass scoped away in `Test/Egress` [src/OcuPilot/Test/EgressLocal.cls:78]
- [x] [Review][Patch] `Base`'s class header still says every family's error body is `{error: {message}}`, the overclaim its own method doc corrects twenty lines below [src/OcuPilot/Kernel/Provider/Base.cls:29]
- [x] [Review][Patch] `Test/Egress`'s header says nothing here writes anything, three lines above the paragraph saying one method writes the outbound-settings row [src/OcuPilot/Test/Egress.cls:6]
- [x] [Review][Patch] The refusal-precedence statement is indented 4 spaces inside an 8-space block [src/OcuPilot/Kernel/Provider/MessageAdapter.cls:239]
- [x] [Review][Patch] AC1's second mutation named `Compatible.ApplyAuth`, which does not exist, while `Test/CompatibleStub` does declare an `ApplyAuth` carrying no such guard [spec `## Verification`]
- [x] [Review][Patch] AC3's local-addresses mutation claims the private-network row reddens; `IsPermitted` answers 1 for `KINDPUBLIC` before it reads either flag [spec `## Verification`]
- [x] [Review][Patch] The origin-guard citation named `OpenAIAdapter`, which declares no such method; the pin is `GeminiAdapter`'s [spec `## Verification`]
- [x] [Review][Patch] AC2's client half had no recorded `mutation:` line, which is what let the two vacuous pins above through (Rule 19) [spec `## Verification`]
- [x] [Review][Patch] The new browser leg is titled AC2 but exercises AC1 and AC3, and asserts `httpAcknowledged === false` [ui/browser/definitions.browser-spec.mjs:431]
- [x] [Review][Patch] The README's privacy sentence says "off the instance" for a model beside it, where data does leave the instance [README.md:38]
- [x] [Review][Patch] Two JSDoc blocks document the declaration below the one they were written for [ui/src/app/areas/agent/definition-form.page.ts:494], [ui/src/app/areas/agent/definition-form.page.spec.ts:59]
- [x] [Review][Defer] `Egress.METADATAADDRESSES` names AWS and GCP only, so Alibaba Cloud's published `100.100.100.200` classifies public and is reachable by any definition [src/OcuPilot/Kernel/Egress.cls:71] -- deferred: real and in-footprint, but widening a security denylist is the orchestrator's call on DW-1214's own precedent, not a reviewer patch. `DW-1217`, `routed owner=range-end-cleanup`
- [x] [Review][Defer] AD-42 does not carry the marked-local proxy bypass this story ships, so the spine still reads that a configured proxy is judged rather than bypassed [ARCHITECTURE-SPINE.md:461] -- deferred: Rule 20 puts the spine write on the lead, and Epic 11 plans against that AD text. `DW-1218`, `routed owner=burndown`
- [x] [Review][Defer] `Test/ContextBound`'s mutation recipe still says the public-proxy legs go red, plural, where DW-441's flip left one [src/OcuPilot/Test/ContextBound.cls:214] -- deferred: the orchestrator granted one literal in that file and named a further change a routed finding, not a patch. `DW-1219`, `routed owner=range-end-cleanup`
- [x] [Review][Defer] A definition may be marked local while naming a public endpoint, so the chip states no egress and `Dispatch` bypasses the proxy for a call that leaves [src/OcuPilot/Kernel/AgentRules.cls:183] -- deferred: filed HIGH by the acceptance auditor, regraded `medium` with high fix-risk and `escalated` -- the reasoning is under `## Review Triage Log`. Already the spec's `deferred:` entry 2; not re-filed.

**Rejected.**

- `false` -- the AC2 component case does tick `markedLocal` before querying the acknowledgment; what it lacked was the intermediate assertion, which the first patch above adds.
- `false` -- `heldCredType` forcing `creds` on a row stored keyless is not harm: there is no prior rung to restore and the form renders no env or creds control to land on.
- `false` -- `.ocu-field-egress` is not a new dark-mode gap: no `--ocu-*` semantic token family is rebound under `:root.ocu-theme-dark`, the toggle is spine `## Deferred` (FR-73), and DW-1216 owns the two-treatments concern.
- `false` -- `TestABaseEndpointIsNormalizedAndAnAuthorityChangeIsRefused` does not over-claim: every leg asserts `FaultCode(tFault) = ""`, which holds only because the built URL kept the endpoint's origin, and the same `Attempts` check refuses an override that answers another authority.
- `false` -- a body key over a plain-`http://` endpoint does not escape the acknowledgment: `HandleTest` runs `Normalize` then `Validate` over the merged stored-plus-body values, so the credential term applies to a body-supplied endpoint too.
- `by-design` -- `Validate` calling `SchemeAccepted` twice is what routes the two refusals to two different fields, which the comment between the calls states.
- `wontfix-theoretical` -- a stale `httpAcknowledged` surviving an https excursion: the control renders ticked whenever it is on screen and `Validate` re-reads the value on every save. `reopen_if=` a save path carries the flag with no control rendered for it.
- `wontfix-theoretical` -- a refusal beside a non-empty `tool_calls`: no vendor documents that shape and DW-1200 forbids the call that would settle it, which is the widening the Design Notes already refuse.
- `wontfix-accepted` -- `Compatible.ProviderName` has no caller: a declared family seam, identical in all four adapters plus the abstract; removing it is a seam-contract change across five files. `reopen_if=` a caller appears.
- `wontfix-accepted` -- `METADATAADDRESSES` has no count pin and two of its four entries are already covered by `IsLinkLocal`: true, but the two DW-1214 added are pinned by name, and a count pin adds a second edit per address. `reopen_if=` an entry is added that no leg names.
- rejected -- the `IsHttps` guard around the tunnel forcing: filed at low confidence as a deletion candidate with no named bad outcome; removing a fail-closed TLS guard to raise branch coverage is the wrong direction.
- rejected (step-03: no finding whose fix edits the spec's frozen intent) -- the "Never" list against the `strings.ts` task, and the matrix row naming `AGENT.ENDPOINT.ADDRESS` where the code answers `AGENT.ENDPOINT.SCHEME`. Both were adjudicated on the same ground in the previous pass; the save is refused either way.
- rejected as churn -- bare `///` parameter prose and a double blank line in `Test/CompatibleAdapter.cls`: the class already mixes both doc forms, and the spec carries `oversized`.
- not re-filed -- four rows restate the spec's existing `deferred:` entries: the key PUT against a `credType` `none` definition (1), the marked-local public endpoint (2), `ValuesFor` not loading `HttpAcknowledged` (4), and `Base.ProviderMessage`'s string error member (5).
...
### 2026-09-19 -- Code review (re-review, Tier 1 `full`, four layers)

46 raw rows across the four layers grouped into 18 entries: 0 high, 7 medium, 11 low. 14 patched in
the pass, 3 ledgered (`DW-1217`, `DW-1218`, `DW-1219`), 1 escalated, 14 rejected with their
refutations under `### Review Findings`. Containment verified: `git status --short` and
`git log OCU-1-epic10 --not --remotes` were unchanged across all four layers, so none of them wrote.

**The one HIGH filed, and why it is a medium.** The acceptance auditor filed the marked-local flag
never being cross-checked against the endpoint's resolved kind as an AD-42 violation. It is not one.
AD-42 makes the declaration the mechanism -- "an explicitly-declared local address" -- and its
"computed from this same configuration, so it cannot disagree" clause is about the computation's
source, which after this story is literally the same `MarkedLocal`, endpoint and proxy that
`Dispatch` reads. What remains is that an operator can declare falsely, which AD-42 does not detect
by design; closing it means deciding whether the declaration is authoritative, and refusing an
unresolvable host would break this story's own private-DNS case. Medium, fix-risk high, `escalated`.

**What the patched mediums have in common.** Three of the four are Rule 19: the row's `allowsLocal`
fixture arrived in the rework, and the three client branches it hosts were then asserted from
starting states that could not tell the fix from the bug -- `httpAcknowledged` never set true before
the cascade, `credType` never anything but `creds` before the untick. The fourth is the same
teardown defect the rework fixed in `Test/Egress` and reproduced in the class it wrote next to it.
All three client mutations were demonstrated on the working tree and reverted byte-identically; the
`EgressLocal` restore was demonstrated positively on a slot-B row seeded for the purpose and removed
afterwards, leaving the instance with the 0 egress rows and 0 agent rows it started with.

**Two refutations worth keeping, so they are not re-filed.** The new browser leg was read as covering
AC2; it covers AC1 and AC3, and its title now says so -- AC2's acknowledgment control has jsdom
coverage only, which is what the spec's task list asked for, so Rule 3 is met by the keyless leg
rather than left open. And `.ocu-field-egress` was read as reproducing DW-1216's dark-mode gap: no
`--ocu-*` semantic token family is rebound under `:root.ocu-theme-dark` and the toggle does not exist
yet, so the rule is consistent with every sibling rather than a new omission.
...
- **AC1 (no API key)** -- `CompatibleAdapter.TestAKeylessCallSendsNoAuthorizationHeader`, driving the shipped row with `credType` `none`. `mutation:` have `ProviderPort.ValuesFor` leave `keySource` unset for a `none` `credType` -> red on the expected call, because `Base.Invoke` then resolves a stored key, finds none and faults `PROVIDER.CREDENTIAL` before any request is built. A second leg, `TestTheAdapterWritesNoHeaderWhenTheKeyIsAbsent`, pins the header itself; `mutation:` drop the empty-key guard (`If ..ApiKey = "" Quit`) from `OpenAI.ApplyAuth:77`, which `Compatible` inherits and does not override -> red on the recorded header names. [corrected at code review 2026-09-19: the recipe named `Compatible.ApplyAuth`, which does not exist, while `Test/CompatibleStub` does declare an `ApplyAuth` that carries no such guard.] A third leg (QA) -- `TestAKeylessRowWithALeftoverReferenceAndAResolvableSecretStillSendsNoHeader`, a row carrying a leftover `envVarName`/`credentialName` and a resolvable armed secret; `mutation:` gate `ProviderPort.ValuesFor`'s `keySource` `none` assignment on both references being empty -> red on the no-fault and one-call assertions, demonstrated on a fresh throwaway (run 399, reverted clean at run 400).
...
- **AC2, client half** -- the three `Story 10.3` cases in `ui/src/app/areas/agent/definition-form.page.spec.ts`, added and made discriminating at code review 2026-09-19. Each mutation was applied to the working tree, `npm run test:components` run, and the tree reverted byte-identically (`git status --short` clean for the mutated file afterwards). `mutation:` drop `if (!this.markedLocalFlag) return false;` from `definition-form.page.ts`'s `showHttpAcknowledge` -> red on `the local controls render only on a row that licenses them...` alone (1 of 644). `mutation:` delete `next['httpAcknowledged'] = false;` from `definition-form.store.ts`'s `applyProviderDefaults` -> red on `cascading off the local row clears the two flags and the keyless rung...` alone. `mutation:` replace `this.heldCredType` with `CRED_TYPE_CREDS` in `onNoKey`'s untick arm, i.e. restore the behavior the rework replaced -> red on the case named "unticking No API key restores an env rung rather than forcing creds" alone.
...
- **AC3 (local addresses)** -- `Egress.TestTheShippedCompatibleRowAllowsLoopbackAndPrivateAndStillRefusesLinkLocal`, over the shipped row rather than the probe. `mutation:` set the `compatible` row's `allowsLocal` false in the `Providers` XData -> red on the method's leading `AllowsLocal` assertion and on the marked-local loopback row, with the unmarked loopback, the private-network and the link-local rows all staying green -- the link-local one because that range has no escape, which is what shows the two are enforced separately. [corrected at code review 2026-09-19: the private-network row stays green under this mutation, because `Egress.IsPermitted` answers 1 for `KINDPUBLIC` before it reads either flag and `10.0.0.5` classifies public; that row is pinned by its own `AssertKind(..., "public")` leg instead.] The link-local arm is pinned by `Egress.TestLinkLocalHasNoEscape`; `mutation:` have `IsPermitted` fall through to the marked-local branch for `KINDLINKLOCAL` -> red on that method alone.
...
- **Endpoint normalization** -- `CompatibleAdapter.TestABaseEndpointIsNormalizedAndAnAuthorityChangeIsRefused`. `mutation:` have `Compatible.RequestUrl` answer the stored endpoint unchanged -> red on the recorded URL for the `/v1` row, the already-complete row staying green. The origin guard's other arm is already pinned by `GeminiAdapter.TestABuiltUrlOnAnotherAuthorityIsRefused` (corrected at code review 2026-09-19; `Test/OpenAIAdapter.cls` declares no such method, as the class doc already says).
...
**(QA) Files touched by the QA pass** -- `src/OcuPilot/Test/CompatibleAdapter.cls` (the third AC1 leg above, an adversarial uppercase-with-trailing-slash endpoint leg on `TestABaseEndpointIsNormalizedAndAnAuthorityChangeIsRefused`, and a doc-comment correction: the authority-mismatch pin this class cites lives in `Test/GeminiAdapter.cls`, not `Test/OpenAIAdapter.cls`, which does not declare that method). AC5 reviewed and confirmed a genuine no-pinning-test-by-construction gap: the QA footprint permits no new `ui/tools/*.test.mjs` file and `strings.test.mjs` asserts authorized string values, not README prose, so no faithful assertion is available there.
