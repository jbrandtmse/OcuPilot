...
**Mutations (Rule 19)** — plan one per AC; apply, observe red, revert, confirm the tree byte-identical, and record the result on the line:

- AC1: `mutation: drop the journalFile assignment from Kernel/Shell/About.Payload -> OcuPilot.Test.UiAboutWire`
- AC2: `mutation: make Kernel/Shell/About.HelpHref answer available=0 for a route whose descriptor names a classic page -> OcuPilot.Test.UiAboutRead and locator-bar.spec.ts`
- AC3: `mutation: drop the screenVerdict filter from Home's Shortcuts block -> home.page.spec.ts`
- Integration AC (DW-3): `mutation: make core/build-identity.ts isStale() return false unconditionally, rebuild, redeploy -> about-help-links.browser-spec.mjs`
- AC5 (no off-origin request): `mutation: replace one link anchor with a fetch of the same URL -> about-help-links.browser-spec.mjs network assertion`
