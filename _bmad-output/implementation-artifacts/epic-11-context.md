...
Make the agent easy to understand for a first-time judge or voter. Done so far: it answers about the screen in view, a first change request ends in a confirmable proposal, replies stream, a proposal names the privilege it needs, and one click explains the current screen. What remains: explain a log or audit entry in one click, offer suggested prompts per screen, and let a citation chip take the user to the row the agent used. Every story changes the same panel transcript render path. The rest run in the order 11.2, 11.3, 11.4 (owner re-order, 2026-09-22).
...
- **The system prompt is one ASCII build-time constant.** Nothing read at runtime is joined to it; a definition override replaces it whole. Its statements: answer from the screen context when it holds the answer, else call a read tool; while read-only, describe the change instead of proposing it; use only the tools offered; when asked to explain the screen, say what it is for, what data it shows and what actions its tools offer, name the read tool in `tools` its data comes from (or say it has none), and propose no change; open the screen under discussion with the navigation tool first. 11.4 replaces the plain-text row-naming sentence.
- **Done work later stories must keep intact:**
  ...
  - Privilege line (11.8): a proposal records at mint the pairs its write's gate requires; its wire row carries `privilege: {requires, missing}`, with `missing` evaluated at each read by Confirm's own check, or `null` when nothing was recorded. The card says "requires", never "sufficient", and Confirm stays enabled. Streamed and plain turns render identical cards.
  - Explain this screen (11.1): the fixed string goes as an ordinary user turn through Send's own path with the screen's context, leaving the draft alone. "Propose no change" is guidance, not a gate: a minted proposal still needs Confirm. Home now posts its identity only (route `""`, screen `shell.home`, `tools` empty); the server accepts the empty route and still enforces namespace and view bounds.
- **Explain an entry (11.2).** Every log viewer and audit-database row has an explain entry point that sends that entry, and only that entry. For an application error only the summary fields go, never the captured variable table (it can hold patient data). Entry text reaches the model as delimited tool-result content, never as instruction.
...
- **Testing.** CI calls no live model: tests pin prompt statements, payload members and defaults; the model's compliance is live-check evidence only. Anything about scroll or geometry needs a browser spec, run against the redeployed bundle. A gate claimed for "every screen" is proven by a registry-driven test, not a sample.
...
- Every user-visible string lives in the client string table and must appear in EXPERIENCE.md's fixed-string table.
- The panel body: banners, then the context chip, then "Explain this screen" (button-text) beneath it, then the transcript. The button is persistent; the greeting, starter prompts and 11.3's suggested prompts show only while the panel is idle and empty.
...
- Builds on Story 4.4 (context payload), 4.5 (turn send), 4.7 (navigation), 4.10 (Home's starter prompts), 4.11 (context chip), Epic 5 (proposal card) and 1.9 (privilege map). 11.4 builds on 11.9's prompt and context members and 11.1's tool-naming reply.
- Epic 12 edits the same panel and turn files in its own regions (proposal outcome, write cards, reply appenders and `PanelWriteCard` in `panel.ts`, parts of `turn.ts` and `proposal-view.ts`); leave those hunks alone.
- 11.2's variable-table rule is the one 11.9 applies to the error list's context.
- Story 14.8's seeded-injection test will run against this prompt; Story 17.7's owner live check consumes 11.1.
