...
- `src/OcuPilot/Api/Router.cls` -- add `<Route Url="/account/password" Method="POST" Call="AccountPasswordChange"/>` between the `/conversation` POST (`:105`) and `/instance` (`:106`), plus the one-line wrapper -- `check_route_ordering` places no constraint on this URL, so the position is house grouping; no 405 guard route is added, because `Router.Http405` `:602` serves them.
...
**Contended and out-of-footprint paths, declared (Rule 11):**
