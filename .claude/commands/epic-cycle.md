...
10. **Slot** (OcuPilot project Rule 21 — a project addition to this skeleton, re-applied after any base-kit re-render): quote the assigned slot's block from `_bmad/custom/parallel.yaml` `slots:` verbatim — `mcp_profile`, `dev_container`, dev ports and every `throwaway` parameter — with the sentence "Every IRIS MCP call you or any stage you spawn makes carries `server: "<mcp_profile>"`; every `smoke.sh --container` and every `docker exec` against the dev instance names `<dev_container>`; every throwaway you or any stage brings up uses exactly these `--dir --project --web --super` values, `OCUPILOT_BROWSER_ORIGIN` and `--container`; every stage prompt you write repeats this block. The profile `ocupilot-iris` does not exist." Record the assignment in `dispatch.yaml` (`slot: <name>`) before the spawn.
...
| `committed … ci=` and the CI resolution points (base kit "CI Is a Gate") | KEPT, in the worktree: record the run at every push, resolve before the next implement spawn (OcuPilot Rule 28; the plan spawn does not wait) and at the burn-down gate, handle a red per the base kit; your completion contract's `ci:` line must be a resolved `success` (or `none`/`unavailable`) — never complete on `pending` or `failure`. The orchestrator re-reads it before the merge gate. |
...
1. **Lead** runs the **burn-down gate** (Rule 17, see "Ledger Drain"): every story-closable (`routed`/`open`) ledger entry owned by `burndown` or by one of this epic's story keys is resolved / made terminal now, chartered into ONE burn-down story `N.<M+1>` (M = the highest existing story number under Epic N in `epics.md` or `sprint-status.yaml`; at most `burndown_story_max` entries, by priority) that runs the full per-story pipeline before the epic closes, or — the explicit overflow — re-owned to a specific next-epic story key. `escalated` / `decision-pending` entries are not work: they are counted and handed to the decision sheet (step 5). Logs `ledger_burndown_complete` (or `ledger_burndown_skipped reason=empty`).
...
- Rule 29 (targeted verification): write `## Verification` as the story's own browser spec files, ObjectScript classes and client tier marked `(loop)`, then the full browser suite and the full ObjectScript sweep marked `(once, before dev_complete)`.
...
```text
Dispatch: invoke /bmad-build-auto via the Skill tool with the argument = the spec file path above (frontmatter status `ready-for-dev`). The skill resumes at its implement step: it captures `baseline_revision`, runs its implementation-handoff subagent against the spec, runs the spec's `## Verification` commands and the Matrix Test Audit (acceptance criteria are judged at its review step), runs its review layers in parallel (two on OcuPilot: verification-gap and intent-alignment, per `_bmad/custom/bmad-build-auto.toml`), triages (`patch` fixes inline; `defer` goes to the spec frontmatter `deferred:` list; `intent_gap` HALTs), then finalizes: `status: done`, followed by ONE local commit of the reviewed diff plus the spec. The tree is clean on entry (the lead verified it) and the skill verifies it is clean on exit.

Rules for this stage (from skill-rules.md):

- Rule 26 (read once): read the spine, the spec, the rules and any diff you generate once and work from the copy you hold; re-read only a region you or a subagent changed since, by line range, never the whole file — and pass this rule into every subagent you spawn.
- Rule 29 (targeted verification): in the handoff, verification, patching and rework passes run only the story's own browser spec files, ObjectScript classes and client tier; run the full browser suite and the full ObjectScript sweep exactly once, before `dev_complete` — pass this into the handoff subagent.
  ...
  Lead: LEDGER slice burndown + slice <each story key of epic N>:
    empty → logs ledger_burndown_skipped reason=empty
            ...
            resolve / terminal what can be closed now; if what remains exceeds ledger_cap (default 8) OR any remaining entry has 2+ occurrences:
              charter Story N.<M+1> "Epic N burn-down" (M = highest existing story number under Epic N across epics.md + sprint-status.yaml; reuse an existing burn-down story) — at most burndown_story_max (default 12) entries by priority (severity, occurrences, low fix-risk first)
...
- **Numbering the burn-down story `N.9`** — 9 was shorthand for "last" and is last only on an epic with eight or fewer stories; "the next unused number" can fill a gap below an existing story. The burn-down story is `N.<M+1>`, M the highest existing story number under Epic N across `epics.md` and `sprint-status.yaml`, reused if it already exists (Rule 17 (3)).
