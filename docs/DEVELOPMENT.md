# OcuPilot: developer reference

This is the contributor-facing reference for the repository: the development container and its start path, the
installer, the readiness endpoint, the smoke script, CI, the IPM manifest, VS Code setup and the IRIS MCP server suite.
For what OcuPilot is and how to install and use it, start with the [README](../README.md).
