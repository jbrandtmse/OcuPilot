...
async function mount(initialRows: unknown[] = [row('OcuPilotDemoTask')]) {
...
describe('task history, across every task', () => {
  it('renders the search field and the checkbox, and reads nothing before Search', async () => {
  ...
  it('Search sends exactly one read carrying search, and renders the answered rows', async () => {
    const { paths, search, typeSearch, host } = await mount([row('OcuPilotDemoTask')]);
    await typeSearch('OcuPilotDemo');
    await search();
    expect(paths).toHaveLength(1);
    expect(paths[0]).toContain('/screens/tasks.history/read?maxRows=1000');
    expect(paths[0]).toContain('&search=OcuPilotDemo');
    expect(paths[0]).not.toContain('userOnly=');
    expect(host.querySelector('[role="grid"]')).not.toBeNull();
    expect(rowNames(host)).toEqual(['OcuPilotDemoTask']);
  });

  it('the checkbox sends userOnly=1 only when checked, and omits it otherwise', async () => {
    const { paths, search, toggleUserOnly } = await mount([]);
    await toggleUserOnly(true);
    await search();
    expect(paths[0]).toContain('&userOnly=1');
  ...
  it('DW-260: Refresh is offered only after the first Search, and re-runs that same search', async () => {
    ...
    expect(actions.has(HISTORY.descriptor, REFRESH_ACTION_ID)).toBe(false);
    await typeSearch('OcuPilotDemo');
    await search();
    expect(paths).toHaveLength(1);
