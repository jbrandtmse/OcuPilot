...
// - skip the trailing-slash drop in `resolveTarget` -> the `/api/admin/` write.
...
const { composeRequest, resolveTarget, refusal, isSafeVerb, maskedRecord, renderBody, TEXT_CAP_BYTES } = tryIt;
...
const verdict = (verb, path) => refusal(verb, resolveTarget(`${ORIGIN}${path}`));
...
test('a composed request that resolves under OcuPilot\'s own application is refused, from the document', () => {
  ...
  assert.equal(refusal(request.method, resolveTarget(request.url)), 'own-application');
